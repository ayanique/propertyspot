...
$shoplayout = 'sidebar';
if(isset($ginza_opt['shop_layout']) && $ginza_opt['shop_layout']!=''){
	$shoplayout = $ginza_opt['shop_layout'];
}
...
$ginza_shop_main_extra_class = NULl;
if($shopsidebar=='left') {
	$ginza_shop_main_extra_class = 'order-lg-last';
}
$main_column_class = NULL;
switch($shoplayout) {
	case 'fullwidth':
		Ginza_Class::ginza_shop_class('shop-fullwidth');
		$shopcolclass = 12;
		$shopsidebar = 'none';
		$productcols = 4;
		break;
	default:
		Ginza_Class::ginza_shop_class('shop-sidebar');
		$shopcolclass = 9;
		$productcols = 3;
...
?>
<div class="main-container shop-page <?php if(isset($ginza_opt['shop_banner']['url']) && ($ginza_opt['shop_banner']['url'])!=''){ echo 'has-image';} ?>">
	<div class="title-breadcumbs">
		<!-- shop banner -->
		<?php if( is_shop()){ ?>
			<?php if(isset($ginza_opt['shop_banner']['url']) && ($ginza_opt['shop_banner']['url'])!=''){ ?>
				<div class="shop-banner banner-image">
					<img src="<?php echo esc_url($ginza_opt['shop_banner']['url']); ?>" alt="<?php esc_attr_e('Shop banner','ginza') ?>" />
				</div>
			<?php } ?>
		<?php } ?>
		<!-- end shop banner -->
		...
		<?php if (is_product_category()) {
			...
			if(isset($ginza_opt['show_category_image']) && $ginza_opt['show_category_image'] == true && $image)  {
	...
	<div class="shop_content">
		<div class="container">
			<div class="shop_content-inner">
				<?php if ( woocommerce_products_will_display() ) { ?>
					<div class="toolbar">
						<div class="toolbar-inner">
							<div class="view-mode">
								<label><?php esc_html_e('View on', 'ginza');?></label>
								<a href="#" class="grid <?php if($ginza_viewmode=='grid-view'){ echo ' active';} ?>" title="<?php esc_attr_e( 'Grid', 'ginza' ); ?>"><?php esc_html_e('Grid', 'ginza');?></a>
								<a href="#" class="list <?php if($ginza_viewmode=='list-view'){ echo ' active';} ?>" title="<?php esc_attr_e( 'List', 'ginza' ); ?>"><?php esc_html_e('List', 'ginza');?></a>
							</div>
							<?php
								/**
				...
				<?php } ?>
				<div class="row">
					<div class="col-12 <?php echo 'col-lg-'.$shopcolclass; ?> <?php echo ''.$main_column_class; ?> <?php echo esc_attr($ginza_shop_main_extra_class);?>">
						<div id="archive-product" class="page-content <?php echo esc_attr($ginza_viewmode);?> ">
							<div class="archive-border">
													...
													foreach($pcategories as $pcategoy) { ?>
														<div class="category-tab col-12 col-sm-6 col-xl-3">
															<?php 
													            ...
													            if ($image) {
													            	echo '<a href=" ' . get_term_link($pcategoy->slug, 'product_cat') . '"> <img src="' . esc_url($image) . '" width = "400" height = "400" alt=" ' . esc_html( $pcategoy->name ) . '" /> </a>';
													            }
															 ?>
								...
								<?php if ( have_posts() ) : ?>	
									...
									<div class="shop-products products <?php echo esc_attr($ginza_viewmode);?> <?php echo esc_attr($shoplayout);?>">

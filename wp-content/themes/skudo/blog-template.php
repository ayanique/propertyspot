...
get_header(); skudo_print_menu();
	...
	function skudo_print_blog($pageid){
			    ...
			    while ($skudo_the_query->have_posts()){
				    ...
				    <article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class($post_classes); ?>>
					    ...
					    <?php
						    ...
						    if (!$posttype || $posttype == "") $posttype = "image";
						    switch($posttype){
					    		case "image":
				    		...
				    		if ($posttype != "quote" && $posttype != "link"){
					    		...
					    		<div class="blog_excerpt">
							    	<?php 
								    	...
								    	printf( '<div class="aligncenter"><a class="aio-icon-read" href="%1$s">%2$s</a></div>',
	    ...
	    <?php
		    ...
		    if ($skudo_animate_posts){
			    ...
			    ';
			    wp_add_inline_script('skudo-global', $animate_posts_script, 'after');
		    }

...
class SkudoOptionsManager{
	...
	var $skudo_images_url='';
	var $skudo_utils_url='';
	var $skudo_uploads_url='';
	var $skudo_version='';
	var $themename='';
	var $first_save='';
	
	/**
	 * The main constructor for the SkudoOptionsManager class
	...
	function __construct($themename, $images_url, $utils_url, $uploads_url, $version){
		...
		$this->skudo_images_url=$images_url;
		$this->skudo_utils_url=$utils_url;
		$this->skudo_uploads_url=$uploads_url;
		$this->skudo_version=$version;
		$this->first_save=get_option("skudo_first_save");
	}
	...
	function print_heading($heading_text){
		...
		echo '<div id="skudo-content-container"><form method="post" id="skudo-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('skudo-theme-update-options','skudo-theme-options');
			wp_nonce_field('skudo-theme-upload-handler','skudo-theme-upload-handler');
		}
	...
	function print_footer(){
		echo '</div></div><div id="skudo-footer"><div id="follow-upper"> 
			 <p>Follow us</p><ul>
			 <li><a href="https://facebook.com/UpperInc" title="Upper on Facebook"><img src="'.esc_url($this->skudo_images_url).'facebook.png" /></a></li>
			 <li><a href="https://twitter.com/UpperInc" title="Upper on Twitter"><img src="'.esc_html($this->skudo_images_url).'twitter.png" /></a></li>
	...
	function print_textarea($value){
		global $allowedtags;
		if ($value['id'] == "skudo_mailchimp_code"){
		...
		$input_value = $this->get_field_value($value['id'], $std);
		if (isset($value['params']) && $value['params'] == 'skudo_rich_editor'){
...
jQuery("#'.esc_js($value['id']).'").html( jQuery("iframe#'.esc_js($value['id']).'_editor_ifr").contents().find("#tinymce.'.esc_js($value['id']).'_editor" ).html() ) });
	...
	function print_multicheck($value){
		...
		if (isset($value['sortable']) && $value['sortable']){
			if (!isset($skudo_admin_inline_script)) $skudo_admin_inline_script = "";
			$skudo_admin_inline_script .= '
				jQuery(document).ready(function(){
					"use strict";
	...
	function print_upload($value){
		...
		echo '<div id="'.esc_attr($value['id']).'_button" class="upload-button upload-logo" ><a class="skudo-button alignright"><span class="upload-panel">Upload</span></a></div><br/>';
		$skudo_admin_inline_script = (isset($skudo_admin_inline_script)) ? $skudo_admin_inline_script : "";
		$skudo_admin_inline_script .= '
			jQuery(document).ready(function(){
				"use strict";
				skudoOptions.loadUploader(jQuery("div#'.esc_js($value['id']).'_button"), "'.esc_js($this->skudo_utils_url).'upload-handler.php", "'.esc_js($this->skudo_uploads_url).'");
			});
		';
		wp_add_inline_script('skudo-admin', $skudo_admin_inline_script, 'after');
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			if($type=='pattern') {
				...
				if($option != "none")
					$style = 'background-image:url('.SKUDO_PATTERNS_URL.$option.');';
				else
			...
			echo '<li onclick="jQuery(this).parents(\'#tab_navigation-2-general\').find(\'#'."skudo".'_style_color\').attr(\'value\',\''.esc_js($option).'\'); jQuery(this).parents(\'#tab_navigation-2-general\').find(\'.color-preview\').css(\'background-color\',\'#'.esc_js($option).'\');" style'.'="'.esc_attr($style).'" class="'.esc_attr($class).'"><a class="style-box" title="'.esc_attr($option).'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_website_loaders($value){
		...
		foreach ($value['options'] as $option) {
			...
			if ($class == "load2" || $class == "load3" || $class == "load6"){
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'."skudo".'_website_loader\').attr(\'value\',\''.esc_js($option['id']).'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.esc_attr($selected).'"><div class="loaders-style-box '.esc_attr($class).'"><div class="loader"></div></div></div>';
			} else {
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'."skudo".'_website_loader\').attr(\'value\',\''.esc_js($option['id']).'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.esc_attr($selected).'"><div class="loaders-style-box loader-inner '.esc_attr($class).'">'.wp_kses_post($divs).'</div></div>';
			}
	...
	function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<div id="'.esc_attr($field['id']).'_button" class="upload-button upload-logo" ><a class="skudo-button alignright"><span>Upload</span></a></div><br/>';
					
					$skudo_admin_inline_script = (isset($skudo_admin_inline_script)) ? $skudo_admin_inline_script : "";
					$skudo_admin_inline_script .= '
						jQuery(document).ready(function(){
							"use strict";
							skudoOptions.loadUploader(jQuery("div#'.esc_js($field['id']).'_button"), "'.esc_js($this->skudo_utils_url).'upload-handler.php", "'.esc_js($this->skudo_uploads_url).'");
						});
					';
					wp_add_inline_script('skudo-admin', $skudo_admin_inline_script, 'after');
			...
			if ($field['id'] == "skudo_google_fonts_name"){
				...
				if (count($fonts) > 1){
					...
					foreach($fonts as $f){
						if ($f != "")
							$skudo_admin_inline_script .= 'jQuery(\'#add_google_font_list\').append(\'<li><b>Font URL: </b><span class="skudo_google_fonts_name_span">'.$f.'</span><br><div class="editButton hover"></div><div class="deleteButton hover"></div></li>\');';
					}	
					$skudo_admin_inline_script .= '
						});
					';
			...
			if ($field['id'] == "skudo_sidebar_name_name"){
				...
				if (count($sidebars) > 0){
					...
					foreach($sidebars as $s){
						if ($s != ""){
							$skudo_admin_inline_script .= 'jQuery(\'#sidebar_name_list\').append(\'<li><b>Name: </b><span class="skudo_sidebar_name_span">'.$s.'</span><br><div class="editButton hover"></div><div class="deleteButton hover"></div></li>\');';	
						}
					}	
					$skudo_admin_inline_script .= '
						});
		...
		echo '<a class="skudo-button custom-option-button" id="'.esc_attr($value['id']).'_button"><span>'.wp_kses_post($value['button_text']).'</span></a>';
		...
		$updir = wp_upload_dir();
		$optsxml = file_exists($updir['basedir']."/options.xml") ? $updir['baseurl']."/options.xml" : get_template_directory_uri() . "/skudo_original_panel_options.xml";

		$skudo_admin_inline_script = (isset($skudo_admin_inline_script)) ? $skudo_admin_inline_script : "";
		$skudo_admin_inline_script .= '
			jQuery(document).ready(function(){
				"use strict";
				skudoOptions.setCustomFieldsFunc("'.esc_js($value['id']).'", ["'.esc_js($idsString).'"], ["'.esc_js($namesString).'"], ['.esc_js($textareaString).'] , "'.esc_js($prv).'",  "");
				jQuery(\'#skudo_export_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().off("click").on("click",function(){
					window.open("'.$optsxml.'");
				});
			});
		';
	...
	function print_goto($value){
		?>
		<div class="des_go_to button"><a target="_blank" href="<?php echo "themes.php?page=skudo_style_options&dgtt=".esc_attr($value['name']); ?>"><?php echo esc_html($value['desc']); ?></a></div>
		<?php
	}
	...
	function print_mediauploader($value){
		$modal_update_href = esc_url( add_query_arg( array(
		    'page' => 'skudo_gallery',
		    '_wpnonce' => wp_create_nonce('skudo_gallery_options'),
	...
	function print_insta_authorize($value){
		...
		$admin_url = admin_url('themes.php?page=skudo_options');
		$theme = "skudo";
		$upper_client_id = "842930362901105";
		$redirect_url = "https://docs.upperthemes.com/instagram-api/index.php";
		$authorize_url = "https://a" . "pi.ins" . "tagr" . "am.com/oauth/authorize/?app_id=".$upper_client_id."&redirect_uri=".$redirect_url."&return_uri={$admin_url}&response_type=code&scope=user_profile,user_media&state={$theme}|||{$admin_url}" ;
		...
		if (isset($_GET['deauthorize_insta']) && $_GET['deauthorize_insta'] != false){
			update_option( 'skudo_insta_token', "" );
			update_option( 'skudo_insta_uid', "" );
			if ($timestamp) wp_unschedule_event( $timestamp, 'upper_insta_cron_hook' );
			echo '<div class="insta_authorization_message">Sucessfully deauthorized.</div>';
		} else {
			$input_value = get_option('skudo_insta_token', false);
			$input_value_uid = get_option('skudo_insta_uid', false);
			if (isset($_GET['access_token'])){
				if (isset($_GET['access_token'])){
					$input_value = $_GET['access_token'];
					update_option( 'skudo_insta_token', $input_value );
				}
				if (isset($_GET['uid'])){
					$input_value_uid = $_GET['uid'];
					update_option( 'skudo_insta_uid', $input_value_uid );
				}
			}
			if ($input_value != false){
				echo '<div class="insta_authorization_message">Sucessfully authorized.</div>';
				$reset_transient = isset($_GET['access_token']) ? true : false;
				if (!$timestamp) wp_schedule_event( time(), 'twicedaily', 'upper_insta_cron_hook' );
				$ig_info = $this->get_iguser_profile($input_value, $input_value_uid, $reset_transient);
				echo '<div class="ig-id ig-label">ID</div><div class="id-id ig-value">'.esc_html($input_value_uid).'</div>';
	...
	function get_iguser_profile($at, $uid, $reset_transient = false) {
		$args = array(
			'fields' => 'id,username',
			'access_token' => $at
		);
		$url = "https://grap" . "h.ins" . "tagr" . "am.com/me";
		
		$response = $this->remote_get($url, $args, "_panel_opts", $reset_transient);
		
		/* outs */
		if (empty($response) || !isset($response['id']) || !isset($response['username'])) {
			return false;
		}
	...
	function remote_get($url = null, $args = array(), $culprit = "", $reset_transient = false) {
		$set_new_trans = false;
		if (!get_transient( 'skudo_insta_transient'.$culprit ) || $reset_transient){
			$url = add_query_arg($args, trailingslashit($url));
			$response = $this->validate_response(wp_remote_get($url, array('timeout' => 29)));
			if (!isset($reponse['error'])) $set_new_trans = true;
		} else {
		...
		if ($set_new_trans){
			set_transient( 'skudo_insta_transient'.$culprit, $response, 3600 );
			$timestamp = wp_get_schedule( 'upper_insta_cron_hook' );
			if (!$timestamp) wp_schedule_event( time(), 'twicedaily', 'upper_insta_cron_hook' );
		}
		return $response;
	}

 ...
 */
class SkudoStyleOptionsManager{
	...
	/**
	 * The main constructor for the SkudoStyleOptionsManager class
	...
	function print_heading($heading_text){
		...
		echo '<div id="skudo-content-container"><form method="post" id="skudo-style-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('skudo-theme-update-style-options','skudo-theme-style-options');
		...
		echo '</ul></div></div><div id="content"><div id="header"><h3 id="theme_name">'.esc_html($this->themename).' <span>v.'.esc_html($this->skudo_version).'</span></h3><a class="online-doc" href="http://docs.upperthemes.com/skudo/" target="_blank"><i class="fa fa-file-text-o"> </i>&nbsp; Online Documentation</a><a class="support" target="_blank" href="http://upperthemes.com/support"><i class="fa fa-comments-o"></i> Help & Support Forum</a></div><input type="submit" value="Save Changes" class="save-button" /><div id="options_container">';
	}
	...
	function print_slider_field($value){
		...
		echo '<div class="slider" title="'.esc_attr($value['id']).'"></div><input class="option-input slider-input slider-input-casethree" name="'.esc_attr($value['id']).'" id="'.esc_attr($value['id']).'" type="text" value="'.esc_attr($input_value).'" />';
		$this->close_option($value);
	}
	...
	function print_color($value){
		...
		if ($params!="no-main-color"){
			echo '<div class="option"><h4 class="padding-bottom-30px" >'.$value['name'].'</h4>';
	...
	function print_upload($value){
		...
		$skudo_admin_inline_script .= '
			jQuery(document).ready(function(){
				"use strict";
				skudo_StyleOptionsManager.loadUploader(jQuery("div#'.esc_js($value['id']).'_button"), "'.esc_js($this->skudo_utils_url).'upload-handler.php", "'.esc_js($this->skudo_uploads_url).'");
			});
		';
	...
	function print_opacity_slider($value){
		echo wp_kses_post($this->before_option_title.$value['name'].$this->after_option_title);
		$input_value = $this->get_field_value($value['id'], $value['std']);
		echo '<div class="slider opacity-slider" title="'.esc_attr($value['id']).'"></div><input class="option-input slider-input slider-input-casefour" name="'.esc_attr($value['id']).'" id="'.esc_attr($value['id']).'" type="text" value="'.esc_attr($input_value).'" />';
		$this->close_option($value);
	}
	...
	function print_upload_from_media($value){
		...
		<div class="uploader" id="upload-<?php echo esc_attr($value['id']); ?>">
		  ...
		  <textarea type="textarea" class="upper_hidden" hidden><?php echo wp_kses_post($input_value); ?></textarea>
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			echo '<li onclick="jQuery(this).parents(\'.sub-navigation-container\').find(\'#'."skudo".'_style_color\').attr(\'value\',\''.esc_js($option).'\'); jQuery(this).parents(\'.sub-navigation-container\').find(\'.color-preview\').css(\'background-color\',\'#'.esc_js($option).'\');" style'.'="'.esc_attr($style).'" class="'.esc_attr($class).'"><a class="style-box" title="'.esc_attr($option).'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					$skudo_admin_inline_script .= '
						jQuery(document).ready(function(){
							"use strict";
							skudo_StyleOptionsManager.loadUploader(jQuery("div#'.esc_js($field['id']).'_button"), "'.esc_js($this->skudo_utils_url).'upload-handler.php", "'.esc_js($this->skudo_uploads_url).'");
						});
					';
					wp_add_inline_script('skudo-admin', $skudo_admin_inline_script, 'after');
			...
			echo '<textarea class="upper_hidden" hidden name="'.esc_attr($field['id']).'" id="'.esc_attr($field['id']).'">'.wp_kses_post($saved_value).'</textarea></div>';
			$field_ids[]=$field['id'];
		...
		$updir = wp_upload_dir();
		$optsxml = file_exists($updir['basedir']."/style_options.xml") ? $updir['baseurl']."/style_options.xml" : get_template_directory_uri() . "/skudo_original_panel_style_options.xml";
		...
		$skudo_admin_inline_script .= '
			jQuery(document).ready(function(){
				...
				jQuery(\'#skudo_export_style_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().off("click").on("click",function(){
					window.open("'.$optsxml.'");
				});
			});
		';

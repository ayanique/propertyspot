var skudoOptions = {
    ...
    init: function (options) {
        skudoOptions.setCheckboxClickHandlers();
        skudoOptions.setHelpFunc();
        skudoOptions.setOnOffFunc();
        skudoOptions.setTextImageFunc();
        skudoOptions.setLeftRightFunc();
        skudoOptions.setLightDarkFunc();
        skudoOptions.setColorpickFunc();
        skudoOptions.setStyleSelectFunc();
        ...
        skudoOptions.setTabs(options.cookie);
        jQuery('#options-submit').on('click', function (event) {
            event.preventDefault();
            jQuery('#skudo-options').submit()
        });
        jQuery('#skudo-content-container').delegate('.hover', 'mouseover', function () {
            jQuery(this).css({
                cursor: 'pointer'
            })
        });
        ...
        jQuery('#skudo-content-container').append('<input type="hidden" value="Upper Options Panel" />')
    },
    ...
    setHelpFunc: function () {
        jQuery('#skudo-content-container').delegate('a.help-button', 'click', function (event) {
            event.preventDefault();
            if (!skudoOptions.dialogOpened) {
                jQuery(this).find('.help-dialog:first').clone().dialog({
                    autoOpen: true,
                    title: jQuery(this).attr('title'),
                    closeText: '',
                    open: function () {
                        skudoOptions.dialogOpened = true
                    },
                    close: function () {
                        skudoOptions.dialogOpened = false
                    }
                })
            }
        })
    ...
    setOnOffFunc: function () {
		var skudo_is_rtl = jQuery('body').hasClass('rtl');
    ...
    loadUploader: function (element, pathToPhp, uploadsUrl, multi) {
        ...
        new AjaxUpload(button, {
			...
			data: {
				...
				security: jQuery('input#skudo-theme-upload-handler').val()
			},
    ...
    showSavedImgData: function (optionsData) {
        ...
        if (optionsData.hiddenIds[i]){
		    for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(skudoOptions.separator)
	        } 
	        for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(skudoOptions.separator)
	        }
	        ...
	        for (var j = 0; j < entryCount - 1; j++) {
	            ...
	            for (var i = 0; i < count; i++) {
	                if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
	                    html += skudoOptions.generatePreview(data[i][j])
	                }
    ...
    setCustomFieldsFunc: function (mainId, fieldIds, labels, istextarea, preview) {
        ...
        skudoOptions.setCommonAddFunc(optionsData)
    },
    setCommonAddFunc: function (optionsData) {
        skudoOptions.showSavedImgData(optionsData);
        jQuery(optionsData.addButton).on("click", function (event) {
            event.preventDefault();
            skudoOptions.addItem(optionsData)
        });
        jQuery(optionsData.ulId).on('sortstop', function (event, ui) {
            skudoOptions.setSliderImgChanges(optionsData)
        });
        skudoOptions.setActionButtonHandlers(optionsData)
    },
    addItem: function (optionsData) {
        ...
        for (var i = 0; i < length; i++) {
            if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
                html += skudoOptions.generatePreview(jQuery(optionsData.inputIds[i]).attr("value"))
            }
    ...
    setSliderImgChanges: function (optionsData) {
        ...
        jQuery(optionsData.ulId + ' li').each(function () {
            for (i = 0; i < count; i++) {
                values[i] += jQuery(this).find('span.' + optionsData.spanClasses[i]).html() + skudoOptions.separator
            }
        });
        for (i = 0; i < count; i++) {
...
var skudo_StyleOptionsManager = {
    separator: '|*|',
    dialogOpened: false,
    init: function (options) {
        skudo_StyleOptionsManager.setCheckboxClickHandlers();
        skudo_StyleOptionsManager.setHelpFunc();
        skudo_StyleOptionsManager.setOnOffFunc();
        skudo_StyleOptionsManager.setTextImageFunc();
        skudo_StyleOptionsManager.setLeftRightFunc();
        skudo_StyleOptionsManager.setLightDarkFunc();
        skudo_StyleOptionsManager.setColorpickFunc();
        skudo_StyleOptionsManager.setStyleSelectFunc();
        jQuery(".sortable").sortable();
        var mainNavOptions = {};
        if (options.cookie) {
            mainNavOptions = {
        ...
        jQuery('#options-submit').on('click', function (event) {
            event.preventDefault();
            jQuery('#skudo-style-options').submit()
        });
        jQuery('#skudo-content-container').delegate('.hover', 'mouseover', function () {
            jQuery(this).css({
                cursor: 'pointer'
		...
		skudo_StyleOptionsManager.setTabs(options.cookie);
    },
    setTabs: function (enableCookies) {
		jQuery('.main-navigation-container').hide();
        var selectedClass = 'ui-tabs-selected',
    ...
    setHelpFunc: function () {
        jQuery('#skudo-content-container').delegate('a.help-button', 'click', function (event) {
            event.preventDefault();
            if (!skudo_StyleOptionsManager.dialogOpened) {
                jQuery(this).find('.help-dialog:first').clone().dialog({
                    autoOpen: true,
                    title: jQuery(this).attr('title'),
                    closeText: '',
                    open: function () {
                        skudo_StyleOptionsManager.dialogOpened = true
                    },
                    close: function () {
                        skudo_StyleOptionsManager.dialogOpened = false
                    }
                })
            }
        })
    ...
    showSavedImgData: function (optionsData) {
        ...
        if (optionsData.hiddenIds[i]){
		    for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(skudo_StyleOptionsManager.separator)
	        } 
	        for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(skudo_StyleOptionsManager.separator)
	        }
	        ...
	        for (var j = 0; j < entryCount - 1; j++) {
	            ...
	            for (var i = 0; i < count; i++) {
	                if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
	                    html += skudo_StyleOptionsManager.generatePreview(data[i][j])
	                }
	                var none = data[i][j] === '' ? '<i>---</i>' : '';
	                html += '<b>' + optionsData.labels[i] + ': </b><span class="' + optionsData.spanClasses[i] + '">' + data[i][j] + '</span>' + none + '<br/>'
	            }
    ...
    setCustomFieldsFunc: function (mainId, fieldIds, labels, istextarea, preview) {
        ...
        skudo_StyleOptionsManager.setCommonAddFunc(optionsData)
    },
    setCommonAddFunc: function (optionsData) {
        skudo_StyleOptionsManager.showSavedImgData(optionsData);
        jQuery(optionsData.addButton).on("click", function (event) {
            event.preventDefault();
            skudo_StyleOptionsManager.addItem(optionsData)
        });
        jQuery(optionsData.ulId).on('sortstop', function (event, ui) {
            skudo_StyleOptionsManager.setSliderImgChanges(optionsData)
        });
        skudo_StyleOptionsManager.setActionButtonHandlers(optionsData)
    },
    addItem: function (optionsData) {
        var length = optionsData.inputIds.length;
        var html = '<li>';
        for (var i = 0; i < length; i++) {
            if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
                html += skudo_StyleOptionsManager.generatePreview(jQuery(optionsData.inputIds[i]).attr("value"))
            }
            html += '<b>' + optionsData.labels[i] + ': </b><span class="' + optionsData.spanClasses[i] + '">' + jQuery(optionsData.inputIds[i]).attr("value") + '</span><br/>'
        }
        html += '<div class="editButton hover"></div><div class="deleteButton hover"></li>';
    ...
    setSliderImgChanges: function (optionsData) {
        ...
        jQuery(optionsData.ulId + ' li').each(function () {
            for (i = 0; i < count; i++) {
                values[i] += jQuery(this).find('span.' + optionsData.spanClasses[i]).html() + skudo_StyleOptionsManager.separator
            }
        });
        for (i = 0; i < count; i++) {
            jQuery(optionsData.hiddenIds[i]).val(values[i])
...
jQuery(window).load(function () {
    if (jQuery('#saved_box').length) {
        setTimeout('skudoOptions.removeSavedMessage()', 3000);
		setTimeout('skudo_StyleOptionsManager.removeSavedMessage()', 3000);
    }
});
	...
	if (jQuery('#upper_single_metas_customtext').length){
		...
		jQuery('#upper_single_display_metas_value').on("change",function(){
			if (jQuery(this).val() == "yes") {
				jQuery('.upper_single_metas').add( jQuery('#upper_single_meta_custom_text_noncename').parent() ).add( jQuery('#upper_single_display_metas_where_value').parent() ).css('display','block'); 
				jQuery('#upper_single_metas_customtext').trigger('change');
			}
			else {
				jQuery('.upper_single_metas').add( jQuery('#upper_single_meta_custom_text_noncename').parent() ).add( jQuery('#upper_single_display_metas_where_value').parent() ).css('display','none');
			}
		}).trigger('change');
...
jQuery(document).on("click", "#skudo_demos_container .theme-browser .theme-actions a", function(){
	...
	if (confirmdemo == true){
		...
		if (jQuery(".skudo_demo_status").data("uiDialog")) jQuery(".skudo_demo_status").dialog("destroy");
		jQuery(".skudo_demo_status").attr("title","Applying the demo").html("<span class=\'spinner is-active\'></span>Installing the theme.<br/>Status:<ul class=\'skudo_demo_progress\'></ul>").dialog({
			modal: true,
			closeOnEscape: false,
			autoOpen: false,
			draggable: false,
		...
		jQuery(".skudo_demo_status").dialog("open");
		jQuery(".skudo_demo_status").data("uiDialog").uiButtonSet.find("button").on("click", function(){
		...
		jQuery(".skudo_demo_status").dialog("option", "title", "Applying the demo - 0%");
		...
		jQuery.retryAjax({
			...
			data: { 
				...
				security: jQuery("input#skudo-theme-options").val()
			},
			success: function(response){
				
				jQuery(".skudo_demo_progress").append("<li>Theme Reactivation [OK]</li>");
				jQuery(".skudo_demo_status").dialog("option", "position", "center");
				
				jQuery(".skudo_demo_status").dialog("option", "title", "Applying the demo - 15%");
				jQuery(".skudo_demo_progress").append("<li class=\'des_step_plugins\'>Installing Plugins...</li>");
				...
				jQuery.retryAjax({
					...
					success: function(response){
						jQuery(".skudo_demo_status").dialog("option", "title", "Applying the demo - 35%");
						jQuery(".skudo_demo_progress .des_step_plugins").html("Plugins Installed [OK]");
						jQuery(".skudo_demo_status").dialog("option", "position", "center");

						// set panel options
						jQuery(".skudo_demo_progress").append("<li class=\'des_step_panels\'>Setting Panels Options...</li>");
						var xmlPath = "https://demos.upperthemes.com/demos/skudo/"+lo_demo+"/options.xml";
						var xmlStylePath = "https://demos.upperthemes.com/demos/skudo/"+lo_demo+"/style_options.xml";
						...
						jQuery.retryAjax({
							...
							success: function (response) {
								jQuery(".skudo_demo_status").dialog("option", "title", "Applying the demo - 60%");
								jQuery(".skudo_demo_progress .des_step_panels").html("Panels Options [OK]");
								jQuery(".skudo_demo_status").dialog("option", "position", "center");
								jQuery(".skudo_demo_progress").append("<li class=\'des_step_contents\'>Importing Contents...</li>");
								var desitimeout = Math.floor(Math.random() * 12) + 3;
								var incre = Math.floor(Math.random() * 2) + 1;
								var perc = 60;
								skudo_import_percentage(desitimeout,incre,perc);
								...
								jQuery.ajax({
									...
									success: function(response){
										...
										jQuery(".skudo_demo_status").dialog("option", "title", "Applying the demo - 90%");
										jQuery(".skudo_demo_progress .des_step_contents").html("Import Contents [OK]");
										jQuery(".skudo_demo_progress").append("<li>Set Menu [OK]</li>");
										jQuery(".skudo_demo_status").dialog("option", "position", "center");
										jQuery(".skudo_demo_progress").append("<li class=\'des_step_widgets\'>Importing Widgets...</li>");
										...
										jQuery.retryAjax({
											...
											success: function(response){
												jQuery(".skudo_demo_status .spinner").removeClass("is-active");
												jQuery(".skudo_demo_progress .des_step_widgets").html("Import Widgets [OK]");
												jQuery(".skudo_demo_status").dialog("option", "position", "center");
												// Reload to complete. 
												jQuery(".skudo_demo_status").append("<p style=\'left:20px; line-height: 15px;\'>Process almost complete.<br/>Click OK to Continue.</p>");
												jQuery("button.ui-button.ui-state-disabled").removeClass("ui-state-disabled");
												jQuery(".skudo_demo_status").dialog("option", "title", "Applying the demo - 100%");
...
function skudo_ajax_error_handler(){ jQuery(".skudo_demo_progress").after("<div class=\'error\'>An unexpected error has occurred. Please <a href=\'#\' onclick=\'javascript:window.location=window.location;\'>refresh</a> the page and try again. If the problem persists, please <a href=\'https://upperthemes.com/support/\'>contact us</a>.</div>"); }
function skudo_import_percentage(t,o,i){window.desigtimeout=setTimeout(function(){i<90?(i+o>89&&(o=89-i),i+=o,jQuery(".skudo_demo_status").dialog("option","title","Applying the demo - "+i+"%"),t=Math.floor(12*Math.random())+5,o=Math.floor(2*Math.random())+1,skudo_import_percentage(t,o,i)):(clearTimeout(window.desigtimeout),t=!1)},1e3*t)}
...
(function(jQuery){jQuery.retryAjax=function(ajaxParams){var errorCallback;ajaxParams.tryCount=(!ajaxParams.tryCount)?0:ajaxParams.tryCount;ajaxParams.retryLimit=(!ajaxParams.retryLimit)?2:ajaxParams.retryLimit;ajaxParams.suppressErrors=true;if(ajaxParams.error){errorCallback=ajaxParams.error;delete ajaxParams.error;skudo_ajax_error_handler()}else{errorCallback=function(){console.warn('ERROR CALLBACK. Please Refresh');};}

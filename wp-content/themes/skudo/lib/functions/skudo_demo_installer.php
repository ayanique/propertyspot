...
function upper_demo_installer(){ 
	...
	if (!wp_verify_nonce($_POST['security'],'skudo-theme-update-options')) return;
	...
	$demosurl = "https://demos.upperthemes.com/demos/skudo/";
	$skudo_stylesheet = get_option('stylesheet') ? get_option('stylesheet') : "skudo";
	...
	switch($_POST['upper_action']){
		...
		case 'dbreset': 
			...
			try {
				...
				$skudositeurl = get_option('siteurl');
				$skudohome = home_url('/');
				...
				if ( $db_tables = $wpdb->get_col("SHOW TABLES LIKE '{$wpdb->prefix}%'") ) {
					...
					$keys['url'] = $skudositeurl;
											
					skudo_wp_update_user($user, $keys);
				}

				switch_theme( $skudo_stylesheet );
				
				update_option('siteurl',$skudositeurl);
				update_option('home',$skudohome);
		...
		case 'install_plugins':
			try{
				$plugins_url = "https://demos.upperthemes.com/plugins/skudo/";
				$plugins = array(
					array('name' => 'skudo_custom_post_types', 'path' => $plugins_url . 'skudo_custom_post_types.zip', 'install' => 'skudo_custom_post_types/skudo_custom_post_types.php'),
					array('name' => 'contact-form-7', 'path' => 'https://downloads.wordpress.org/plugin/contact-form-7.5.1.zip', 'install' => 'contact-form-7/wp-contact-form-7.php'),
					array('name' => 'really-simple-captcha', 'path' => 'https://downloads.wordpress.org/plugin/really-simple-captcha.2.0.1.zip', 'install' => 'really-simple-captcha/really-simple-captcha.php'),
					array('name' => 'LayerSlider', 'path' => $plugins_url . 'LayerSlider.zip', 'install' => 'LayerSlider/layerslider.php'),
				...
				$plgs = skudo_get_plugins($plugins);
			} catch (Exception $e){
				$errors = $e->getMessage();
			}
			flush();
		...
		case 'import_content_set_options':
			...
			try{
				...
				// import content
				skudo_autoimport($demosurl, $demo);
				...
				update_option("theme_mods_".$skudo_stylesheet, $mods);
				update_option("mods_skudo", $mods);
		...
		case 'import_widgets':
			...
			ob_start();
			skudo_wie_process_import_file($demosurl . $demo . "/" . $filename);
			ob_end_clean();
			if ( !defined('RS_PLUGIN_PATH') ) skudo_plugin_activate('revslider/revslider.php');
			flush();
			ob_flush();
			sleep(1);
			wp_send_json_success( $errors, 200 );
		...
		case 'complete-installation':
			...
			//rev
			try{
				if (function_exists('skudo_add_increase_time')) skudo_add_increase_time();
				
				$dir = "https://demos.upperthemes.com/demos/skudo/" . $_POST['upper_demo'] . "/revdemos/";
				...
				if ($thefile != ""){
					...
					foreach($zips as $zip){
						if ($zip != ""){
							...
							if (!skudo_add_imported_id( $slug, true )) continue;
				...
				$cubefp = "https://demos.upperthemes.com/demos/skudo/" . $_POST['upper_demo'] . "/cubeportfolio.json";
...
function skudo_wie_process_import_file( $file ) {
	WP_Filesystem();
	...
	if ($data){
		ob_start();
		skudo_wie_import_data( $data );
		ob_end_clean();		
	}
}

function skudo_wie_available_widgets() {
...
function skudo_wie_import_data( $data ) {

	global $wp_registered_sidebars;
	$available_widgets = skudo_wie_available_widgets();
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = esc_html__( 'Site does not support widget', 'skudo' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = esc_html__( 'Widget already exists', 'skudo' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = esc_html__( 'Imported', 'skudo' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = esc_html__( 'Imported to Inactive', 'skudo' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'skudo' ); // show "No Title" if widget instance is untitled
 ...
 */
function skudo_wp_update_user($user, $keys) {
	global $wpdb;			
...
function skudo_get_plugins($plugins){
	$args = array(
	...
	foreach($plugins as $plugin){
		...
		//if (!file_exists(ABSPATH.'wp-content/plugins/'.$plugin['install'])){
			skudo_plugin_download($plugin['path'], $args['path'].$plugin['name'].'.zip');
			skudo_plugin_unpack($args, $args['path'].$plugin['name'].'.zip');
		//}
		/* leave revslider for later. */
		if ($plugin['name'] != "revslider") {
			$output[] = $plugin;
			skudo_plugin_activate($plugin['install']);
		}
	}
	return $output;
}

function skudo_plugin_download($url, $path){
...
function skudo_plugin_unpack($args, $target){
	global $wp_filesystem;
...
function skudo_plugin_activate($installer){

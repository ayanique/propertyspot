...
if ( isset($_GET['page']) && $_GET['page'] == SKUDO_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'skudo_init_options_functionality');  
	add_action('init', 'skudo_set_options'); 
}

if ( isset($_GET['page']) && $_GET['page'] == SKUDO_STYLE_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'skudo_init_style_options_functionality');  
	add_action('init', 'skudo_set_style_options'); 
}
 ...
 */
function skudo_init_style_options_functionality(){
	global $skudo_style_options;

	$skudo_style_options=array();

	//get all the categories
	$categories=get_categories('hide_empty=0');
	$skudo_categories=array();
	for($i=0; $i<sizeof($categories); $i++){
		$skudo_categories[]=array('id'=>$categories[$i]->cat_ID, 'name'=>$categories[$i]->cat_name);
	}

	//load the files that contain the options
	$skudo_style_options_files=array('style-general', 'style-website-loading', 'style-header-top-bar', 'style-logotype', 'style-menu', 'style-sliding-panel', 'style-search', 'style-pagetitle', 'style-footer', 'style-typography' , 'style-import-export');
	foreach($skudo_style_options_files as $file){
		require_once(SKUDO_OPTIONS_PATH.$file.'.php');
	}
}
...
function skudo_set_style_options(){
	...
	if ( $_GET['page'] == SKUDO_STYLE_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['skudo-theme-style-options'],'skudo-theme-update-style-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("skudo_style_first_save")){
						update_option("skudo_style_first_save",'true');
					}
					...
					$doc->save($filename);		
					header("Location: themes.php?page=".SKUDO_STYLE_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 * to populate the global options array.
 */
function skudo_init_options_functionality(){
	global $skudo_options;
	...
	//load the files that contain the options
	$skudo_options_files=array('general', 'header-menu', 'widgetsarea', 'sliders', 'translation', 'social', 'data');
	if (defined('SKUDO_PLG_ACTIVE') === true) $skudo_options_files = array('general', 'header-menu', 'widgetsarea', 'sliders', 'newsletter', 'translation', 'social', 'customcss' , 'data');
	
	foreach($skudo_options_files as $file){
		require_once(SKUDO_OPTIONS_PATH.$file.'.php');
	}
}
 ...
 * Sets the Options save functionality.
 */
function skudo_set_options(){
	
	wp_enqueue_script( 'upper-ace-rich-editor', get_template_directory_uri().'/lib/classes/ace/min/ace.js', array('jquery'), $in_footer = false );
	...
	//insert the default values if the fields are empty
	foreach ($skudo_options as $value) {
		if(isset($value['id'])){
			if(get_option($value['id'])=='' && isset($value['std']) && !in_array($value['type'], $nonsavable_types)){
				update_option( $value['id'], $value['std']);
			}
	...
	//save the field's values if the Save action is present
	if ( $_GET['page'] == SKUDO_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['skudo-theme-options'],'skudo-theme-update-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("skudo_first_save")){
						update_option("skudo_first_save",'true');
					}
					...
					foreach ($skudo_options as $value) {
						...
						if (isset($value['id'])){
							if( isset( $_REQUEST[ $value['id'] ] ) && !in_array($value['type'], $nonsavable_types)) {
								...
								if ($value['id'] === "skudo_portfolio_permalink"){
									$oldval = get_option("skudo_portfolio_permalink");
								...
								// reset transients on limit change
								if ($value['id'] == 'skudo_instagram_limit'){
									$oldval = get_option('skudo_instagram_limit');
									$newval = $_REQUEST[$value['id']];
									if ($oldval !== $newval){
										delete_option('_transient_timeout_skudo_insta_transient_scraper');
										delete_option('_transient_skudo_insta_transient_scraper');
										delete_option('_transient_timeout_skudo_insta_transient_user_items');
										delete_option('_transient_skudo_insta_transient_user_items');
									}
								}
					...
					$doc->appendChild($root);
					$doc->save($filename);		
					header("Location: themes.php?page=".SKUDO_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 */
function skudo_theme_admin() {
	global $skudo_options,$skudo_options_manager;

	$skudo_options_manager=new SkudoOptionsManager("Skudo", SKUDO_IMAGES_URL, SKUDO_UTILS_URL, SKUDO_UPLOADS_URL, SKUDO_VERSION);
	$skudo_options_manager->set_options($skudo_options);

	if ( isset($_REQUEST['saved'] )) {
		$skudo_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$skudo_options_manager->print_reset_message();
	}

	$skudo_options_manager->print_heading("");
	$skudo_options_manager->print_options();
	$skudo_options_manager->print_footer();
}
 ...
 */
function skudo_theme_style_options_admin() {
	global $skudo_style_options, $skudo_style_options_manager;

	$skudo_style_options_manager=new SkudoStyleOptionsManager("Skudo", SKUDO_IMAGES_URL, SKUDO_UTILS_URL, SKUDO_UPLOADS_URL, SKUDO_VERSION);
	$skudo_style_options_manager->set_options($skudo_style_options);

	if ( isset($_REQUEST['saved'] )) {
		$skudo_style_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$skudo_style_options_manager->print_reset_message();
	}

	$skudo_style_options_manager->print_heading("");
	$skudo_style_options_manager->print_options();
	$skudo_style_options_manager->print_footer();
}
 ...
 */
function skudo_theme_demos_admin() {
	global $skudo_demos_manager;

	$skudo_demos_manager=new SkudoDemosManager("Skudo", SKUDO_IMAGES_URL, SKUDO_UTILS_URL, SKUDO_UPLOADS_URL, SKUDO_VERSION);

	$skudo_demos_manager->print_heading("");
	$skudo_demos_manager->print_options();
	$skudo_demos_manager->print_footer();
}
 ...
 */
function skudo_add_options($option_arr){
	global $skudo_options;

	foreach($option_arr as $option){
		$skudo_options[]=$option;
	}
}

function skudo_add_style_options($option_arr){
	global $skudo_style_options;

	foreach($option_arr as $option){
		$skudo_style_options[]=$option;
	}
}
 ...
 */
function skudo_get_opt($option){
	return stripslashes(get_option("skudo".$option));
}


function skudo_get_custom_sidebars(){
	$res=array();
	$sides=get_option('skudo_sidebar_name_names');
	if($sides){
		$res=explode(SKUDO_SEPARATOR, $sides);
		array_pop($res);
	}
	return $res;
}
 ...
 */
function skudo_add_custom_options($opt_array){
	$new_skudo_options=array();
	...
	foreach($opt_array as $option){
		if($option['type']=='multiple_custom'){
			...
			$saved_array=explode(SKUDO_SEPARATOR, $saved_values);
			if(sizeof($saved_array)>1){
				array_pop($saved_array);
				foreach($saved_array as $custom_name){
					$id=skudo_convert_to_class($custom_name);
					$custom_option=array(
					"id"=>$id,
					...
					array_push($new_skudo_options, $custom_option);
				}
			}
				
		} else{
			//this is just a normal option, just append it into the new array
			array_push($new_skudo_options, $option);
		}
	}

	return $new_skudo_options;
}

function skudo_fonts_array_builder(){
	$defaultfonts = array();
...
function skudo_portfolio_types(){
...
function skudo_get_proj(){
	
	$skudo_proj2=array(array('id'=>'default', 'name'=>'Default'));
	if (function_exists('skudo_get_projects')){
		$skudo_proj1 = skudo_get_projects();
		foreach($skudo_proj1 as $dp){
			$skudo_proj2[]=array("name"=>$dp['p_title'], "id"=>$dp['p_id']);
		}
	}
...
function skudo_get_all_google_fonts(){
	$skudo_google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),

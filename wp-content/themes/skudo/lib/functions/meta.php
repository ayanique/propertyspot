 ...
 * ADD THE ACTIONS
 */
add_action('admin_menu', 'skudo_load_meta_boxes');
add_action('admin_menu', 'skudo_create_meta_box');  
add_action('admin_menu', 'skudo_create_meta_post_box');  
add_action('admin_menu', 'skudo_create_meta_portfolio_box'); 
add_action('admin_menu', 'skudo_create_meta_testimonials_box');  
add_action('admin_menu', 'skudo_create_meta_partners_box');
add_action('admin_menu', 'skudo_create_meta_team_box');
add_action('save_post', 'skudo_save_postdata');  
add_action('save_post', 'skudo_save_post_postdata');  
add_action('save_post', 'skudo_save_portfolio_postdata'); 
add_action('save_post', 'skudo_save_testimonials_postdata');
add_action('save_post', 'skudo_save_partners_postdata');
add_action('save_post', 'skudo_save_team_postdata');
add_action('admin_footer','skudo_print_helper');

if (!defined('SKUDO_PORTFOLIO_POST_TYPE')){
	$portfolio_permalink = get_option("skudo_portfolio_permalink");
	if (!get_option("skudo_portfolio_permalink")) define("SKUDO_PORTFOLIO_POST_TYPE", "portfolio");
	else define("SKUDO_PORTFOLIO_POST_TYPE", get_option("skudo_portfolio_permalink"));
}
...
if (isset($_REQUEST['file'])) { 
    $options = get_option('skudo_gallery_options', TRUE);
	if (isset($options['default_image'])) $options['default_image'] = absint($_REQUEST['file']) ? absint($_REQUEST['file']) : "";
    update_option('skudo_gallery_options', $options);
}


function skudo_load_meta_boxes(){
	//load the porftfolio categeories
	$sidebar_taxonomies=skudo_get_custom_sidebars();
	...
	foreach($sidebar_taxonomies as $taxonomy){
		$sidebar_categories[]=array("name"=>$taxonomy, "id"=>skudo_convert_to_class($taxonomy));
	}
	
	//load the porftfolio categeories
	$portf_categories=array(array('id'=>'all', 'name'=>'All Portfolios'));
	if (function_exists('skudo_get_taxonomies')){
		$portf_taxonomies=skudo_get_taxonomies('portfolio_type');
	...
	global $skudo_data, $skudo_new_meta_boxes, $skudo_new_meta_portfolio_boxes, $skudo_new_meta_testimonials_boxes, $skudo_new_meta_post_boxes, $skudo_new_meta_partners_boxes, $skudo_new_meta_team_boxes;
		 ...
		 * ------------------------------------------------------------------------*/
		 ...
		 if (get_option("skudo_body_type") == "pattern") $varType = SKUDO_PATTERNS_URL.get_option("skudo_body_pattern");
		 else $varType =  get_option("skudo_header_body_pattern");
	
		//the meta data for pages
		$skudo_new_meta_boxes =
		array(
		...
		array(
			...
			"name" => "skudo_enable_custom_header_options",
			"type" => "select",
			"options" => array(array("id"=>"yes","name"=>"Yes, please."), array("id"=>"no", "name"=>"No, thanks.")),
			"std" => "no"
		),
		...
		array(
			...
			"name" => "skudo_enable_website_loading",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."), array("id"=>"off", "name"=>"No, thanks.")),
			"std" => get_option('skudo_enable_website_loader'),
			"description" => "If set to YES, you'll need to activate the 'Enable Website Loading' options in <strong>Appearance</strong> > <strong>Skudo Options</strong> > <strong>General</strong> > <strong>Main Settings</strong> to choose the Loader Style and Save the options. You can then deactive the option again."
		),
		
		array(
			"title" => "Page Content Starts...",
			"name" => "skudo_content_to_the_top",
			"type" => "select",
			"options" => array(array("id"=>"off","name"=>"After the header"), array("id"=>"on","name"=>"Behind the header")),
			"std" => get_option('skudo_content_to_the_top')
		),
		
		array(
			"title" => "Header Pre Scroll Style",
			"name" => "skudo_custom_header_pre",
			"type" => "select",
			"options" => array(array("id"=>"light","name"=>"Light"), array("id"=>"dark", "name"=>"Dark")),
			"std" => get_option('skudo_header_style_light_dark') != "" ? get_option('skudo_header_style_light_dark') : "light"
		),
		
		array(
			"title" => "Header Pre Scroll Transparent ?",
			"name" => "skudo_custom_header_pre_transparent",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."), array("id"=>"off", "name"=>"No, thanks.")),
			"std" => "off"
		),
		...
		array(
			...
			"name" => "skudo_custom_header_after",
			"type" => "select",
			"options" => array(array("id"=>"light","name"=>"Light"), array("id"=>"dark", "name"=>"Dark")),
			"std" => get_option('skudo_header_after_scroll_style_light_dark') != "" ? get_option('skudo_header_after_scroll_style_light_dark') : "light"
		),
		
		array(
			"title" => "Enable Custom Page Title Options ?",
			"name" => "skudo_enable_custom_pagetitle_options",
			"type" => "select",
			"options" => array(array("id"=>"yes","name"=>"Yes, please."), array("id"=>"no", "name"=>"No, thanks.")),
			"std" => "no"
		),
		...
		array(
			...
			"std" => get_option('skudo_header_type')
		),
		
		array(
			"title" => "Image",
			"name" => "skudo_header_image",
			"type" => "mediaupload",
		...
		array(
			...
			"name" => "skudo_header_background_position",
			"type" => "select",
			"options" => array(array('id'=>'top', 'name'=>'Top'), array('id'=>'center','name'=>'Center'), array('id'=>'bottom','name'=>'Bottom')),
			"std" => get_option('skudo_header_background_position'),
			"description" => "Only available if Parallax is not active."
		),
		
		array(
			"title" => "Parallax ?",
			"name" => "skudo_pagetitle_image_parallax",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."),array("id"=>"off","name"=>"No, thanks.")),
			"std" => "off",
		),
		...
		array(
			"title" => "Overlay ?",
			"name" => "skudo_pagetitle_image_overlay",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."),array("id"=>"off","name"=>"No, thanks.")),
			"std" => "off"
		),
		...
		array(
			"title" => "Overlay Type",
			"name" => "skudo_pagetitle_overlay_type",
			"type" => "select",
		...
		array(
			"title" => "Overlay Color",
			"name" => "skudo_pagetitle_overlay_color",
			"type" => "color",
			"std" => "333333"
		),
		...
		array(
			...
			"name" => "skudo_pagetitle_overlay_pattern",
			"type" => "pattern",
			"options" => $patterns,
		),
		...
		array(
			...
			"name" => "skudo_pagetitle_overlay_opacity",
			"type" => "slider",
			"std" => "100%"
		),
		// end of new options
		...
		array(
			"title" => "Color",
			"name" => "skudo_header_color",
			"type" => "color"
		),
		
		array(
			"title" => "Color Opacity",
			"name" => "skudo_header_color_opacity",
			"type" => "slider",
			"std" => "100"
		),
		...
		array(
			...
			"name" => "skudo_header_custom_pattern",
			"type" => "mediaupload",
		...
		array(
			"title" => "Banner Slider",
			"name" => "skudo_banner_slider",
			"type" => "select",
			"options" => skudo_get_created_camera_sliders()
		),

		array(
			"title" => "Page Title Padding",
			"name"=> "skudo_page_title_padding",
			"type" => "text",
			"std" => "50px"
		),
		...
		array(
			...
			"name" => "skudo_header_text_alignment",
			"type" => "select",
			"std" => "left",
		...
		array(
			...
			"name" => "skudo_header_text_font",
			"type" => "select",
			"options" => skudo_fonts_array_builder(),
		...
		array(
			...
			"name" => "skudo_header_text_margin_top",
			"type" => "text",
			"std" => "20px"
		),
		...
		array(
			...
			"name" => "skudo_secondary_title_font",
			"type" => "select",
			"options" => skudo_fonts_array_builder(),
			"description" => 'You can select one of the fonts that the theme goes with or you can add google fonts (Style Options > Fonts).',
			"std" => 'Helvetica Neue'
		...
		array(
			...
			"name" => "skudo_secondary_title_text_color",
			"type" => "color",
			"std" => "828282"
		),
		...
		array(
			...
			"name" => "skudo_secondary_title_text_size",
			"type" => "text",
			"std" => "12px"
		),
		...
		array(
			...
			"name" => "skudo_header_secondary_text_margin_top",
			"type" => "text",
			"std" => "10px"
		),
		...
		array(
			...
			"name" => "skudo_breadcrumbs_margin_top",
			"type" => "text",
			"std" => "10px"
		),
			...
			array(
				"title" => "Choose the Revolution, Master or Layer Slider",
			...
			array(
				...
				"name" => "homeYoutubePlaceholder",
				"type" => "mediaupload_standard",
				"description"=> "Placeholder image is displayed in case background videos are restricted."
			),
		...
		array(
			...
			array(
				...
				"description" => 'You can choose from the following post types: Featured Image, Slider, Gallery, Video, Audio, Text, Quote, Link or None.'
			),
			
			array(
				"title" => "Display Only Feature Image on Blog Masonry",
				"name" => "skudo_display_featured_image_only",
				"type" => "select",
				"options" => array(array("id"=>"yes","name"=>"Yes, please."), array("id"=>"no", "name"=>"No, thanks.")),
				"std" => "no"
			),
			...
			array(
				...
				"name" => "skudo_display_featured_image",
				"type" => "select",
				"options" => array(array("id"=>"yes","name"=>"Yes, please."), array("id"=>"no", "name"=>"No, thanks.")),
				"std" => "yes"
			),
		...
		array(
			"title" => "Display Metas Where ?",
			"name" => "upper_single_display_metas_where",
			"type" => "select",
			"options" => array(array("id"=>"pagetitle","name"=>"Page Title section"), array("id"=>"content", "name"=>"Content")),
			"std" => "content"
		),
		...
		$skudo_new_meta_portfolio_boxes =
		array(
			...
			array(
				...
				"std" => get_option("skudo_projs_flex_navigation"),
				"description" => ""
			),
			...
			array(
				...
				"type" => "select",
				"options" => array(array("name"=>"Yes", "id"=>"true"), array("name"=>"No", "id"=>"false")),
				"std" => get_option("skudo_projs_flex_controls"),
				"description" => ""
			),
			...
			array(
				...
				"std" => get_option("skudo_projs_flex_transition"),
				"description" => ""
			),
			...
			array(
				...
				"type" => "text",
				"std" => get_option("skudo_projs_flex_transition_duration"),
			...
			array(
				...
				"type" => "text",
				"std" => get_option("skudo_projs_flex_slide_duration"),
			...
			array(
				...
				"type" => "select",
				"options" => array(array("name"=>"Yes", "id"=>"true"), array("name"=>"No", "id"=>"false")),
				"std" => get_option("skudo_projs_flex_autoplay"),
				"description" => ""
			),
			...
			array(
				...
				"type" => "select",
				"options" => array(array("name"=>"Yes", "id"=>"true"), array("name"=>"No", "id"=>"false")),
				"std" => get_option("skudo_projs_flex_pause_hover"),
			...
			array(
				...
				"type" => "text",
				"std" => get_option("skudo_projs_flex_height"),
			...
			array(
				...
				"name" => "skudo_custom_header_pre_transparent",
				"type" => "select",
				"options" => array(array("id"=>"off", "name"=>"No, thanks."), array("id"=>"on","name"=>"Yes, please.")),
				"std" => "off"
			),
		...
		$skudo_new_meta_testimonials_boxes =
		array(
		...
		$skudo_new_meta_team_boxes = array();
}
 ...
 * Creates a post meta box.
 */
function skudo_create_meta_portfolio_box() {
	if ( function_exists('add_meta_box') && defined('SKUDO_PORTFOLIO_POST_TYPE')) {
		add_meta_box( 'new-meta-portfolio-boxes', '<div class="icon-small"></div> '."Skudo".' PORTFOLIO ITEM SETTINGS', 'skudo_new_meta_portfolio_boxes', SKUDO_PORTFOLIO_POST_TYPE, 'normal', 'high' );
	}
}
 ...
 */
function skudo_create_meta_partners_box() {
	if ( function_exists('add_meta_box') && defined('SKUDO_PARTNERS_POST_TYPE')) {
		add_meta_box( 'new-meta-partners-boxes', '<div class="icon-small"></div> '."Skudo".' PARTNERS ITEM SETTINGS', 'skudo_new_meta_partners_boxes', SKUDO_PARTNERS_POST_TYPE, 'normal', 'high' );
	}
}

function skudo_create_meta_team_box() {
	if ( function_exists('add_meta_box') ) {
 ...
 */
function skudo_new_meta_boxes() {
	global $post, $skudo_new_meta_boxes;

	foreach($skudo_new_meta_boxes as $meta_box) {
		skudo_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function skudo_new_meta_post_boxes() {
	global $post, $skudo_new_meta_post_boxes;

	foreach($skudo_new_meta_post_boxes as $meta_box) {
		skudo_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function skudo_new_meta_portfolio_boxes() {
	global $post, $skudo_new_meta_portfolio_boxes;

	foreach($skudo_new_meta_portfolio_boxes as $meta_box) {
		skudo_print_meta_box($meta_box, $post);
	}
}
 ...
 * Calls the print method for portfolio meta boxes.
 */
function skudo_new_meta_testimonials_boxes() {
	global $post, $skudo_new_meta_testimonials_boxes;

	foreach($skudo_new_meta_testimonials_boxes as $meta_box) {
		skudo_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function skudo_new_meta_partners_boxes() {
	global $post, $skudo_new_meta_partners_boxes;

	foreach($skudo_new_meta_partners_boxes as $meta_box) {
		skudo_print_meta_box($meta_box, $post);
	}
}
 ...
 * Calls the print method for partners meta boxes.
 */
function skudo_new_meta_team_boxes() {
	global $post, $skudo_new_meta_team_boxes;

	foreach($skudo_new_meta_team_boxes as $meta_box) {
		skudo_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function skudo_print_meta_box($meta_box, $post){
		...
		case 'multicheck':
			...
			if(sizeof($meta_box['options'])>0){
				...
				$skudo_admin_inline_script .= '
					jQuery(document).ready(function(){
						...
						function update_multicheck_'.esc_js($mcindex).'(){
							...
							jQuery("#skudo-multicheck-'.esc_js($mcindex).'").attr("value",multicheck_value);
							var allselected = true;
							for (var i = 0; i < jQuery("#multicheck-'.esc_js($mcindex).' > label").not(".button").length; i++){
								if (jQuery("#multicheck-'.esc_js($mcindex).' label").not(".button").eq(i).children("input").attr("checked") == undefined) allselected = false;
							}
				...
				if (isset($meta_box['sortable']) && $meta_box['sortable']){
					...
					<div class="multicheck-sorter" data-rel="skudo-multicheck-<?php echo esc_attr($mcindex); ?>">
						<?php
							foreach ($meta_box['options'] as $option) { ?>
					...
					$skudo_admin_inline_script .= '
						function upper_multicheck_sortable_'.esc_js($mcindex).'(){
							jQuery(".multicheck-sorter[data-rel=\"skudo-multicheck-'.esc_js($mcindex).'\"] .multicheck-sorter-item").removeClass("enable");
							var values = jQuery("#skudo-multicheck-'.esc_js($mcindex).'").val().split(",");
						...
						jQuery(document).ready(function(){
							...
							var initialVals = jQuery("#skudo-multicheck-'.esc_js($mcindex).'").val();
							if (initialVals != ""){
								initialVals = initialVals.split(",");
								initialVals.reverse();
	
								for (var i=0; i<initialVals.length; i++){
									jQuery(".multicheck-sorter[data-rel=\"skudo-multicheck-'.esc_js($mcindex).'\"]").prepend( jQuery(".multicheck-sorter[data-rel=\"skudo-multicheck-'.esc_js($mcindex).'\"] .multicheck-sorter-item[for=\"'.esc_js($meta_box['name']).'_"+initialVals[i]+"\"]") );
								}
							}
							
							jQuery(".multicheck-sorter[data-rel=\"skudo-multicheck-'.esc_js($mcindex).'\"]").sortable({
								axis: "x",
								stop: function(event,ui){
									var newVal = "";
									jQuery(".multicheck-sorter[data-rel=\"skudo-multicheck-'.esc_js($mcindex).'\"] .multicheck-sorter-item.enable").each(function(){
										if (newVal != "") newVal += ",";
										newVal += jQuery(this).attr("name");
									});
									jQuery("#skudo-multicheck-'.esc_js($mcindex).'").val(newVal);
								}
							});
						});
			...
			echo'<input type="text" name="'.esc_attr($meta_box['name']).'_value" value="'.esc_attr($meta_box_value).'" id="skudo-multicheck-'.esc_attr($mcindex).'" class="option-input hidden" />';
			echo '</div>';
			$mcindex++;
		break;
		case 'heading':
...
<h4>'.wp_kses_post($meta_box['title']).'</h4></div>';
		...
		case 'color':
			echo '<div class="option-container skudo-color-option">';
			echo'<input type="hidden" name="'.esc_attr($meta_box['name']).'_noncename" id="'.esc_attr($meta_box['name']).'_noncename" value="'.wp_create_nonce( plugin_basename(__FILE__) ).'" />';
		...
		case 'upload':
			...
			echo'<input type="text" name="'.esc_attr($meta_box['name']).'_value" value="'.esc_attr($meta_box_value).'" id="skudo-'.esc_attr($meta_box['name']).'" class="option-input upload"/>';

			echo '<div id="skudo-'.esc_attr($meta_box['name']).'_button" class="upload-button upload-logo" ><a class="button button-upload"><span>Upload</span></a></div><br/>';

			$skudo_admin_inline_script = (isset($skudo_admin_inline_script)) ? $skudo_admin_inline_script : "";
			$skudo_admin_inline_script .= '
				jQuery(document).ready(function(){
					"use strict";
					skudoOptions.loadUploader(jQuery("div#skudo-'.esc_js($meta_box['name']).'_button"), "'.SKUDO_UTILS_URL.'upload-handler.php", "'.SKUDO_UPLOADS_URL.'");
				});
			';
		...
		case 'select':
			echo '<div class="option-container'; 
			if ($meta_box['name'] == 'skudo_enable_custom_header_options' || $meta_box['name'] == 'skudo_enable_custom_pagetitle_options') echo ' skudo_folding_options ';
			echo '">';
			echo'<input type="hidden" name="'.esc_attr($meta_box['name']).'_noncename" id="'.esc_attr($meta_box['name']).'_noncename" value="'.wp_create_nonce( plugin_basename(__FILE__) ).'" />';
		...
		case 'pattern':
			...
			if(sizeof($meta_box['options'])>0){
				foreach ($meta_box['options'] as $option) { 
					if ($option['name'] != "none"){
						?> <div onclick="jQuery(this).addClass('selected').siblings().removeClass('selected');jQuery(this).parent().siblings('select').val('<?php echo esc_js($option['id']); ?>');" <?php echo ' style'.'="background-image:url('.esc_url(get_template_directory_uri()).'/images/skudo_patterns/'.esc_attr($option['name']).');" '; ?> class="pattern_item
						<?php if ( $meta_box_value == $option['id']) {
							echo ' selected';
						}
						echo '"';
 ...
 */
function skudo_save_postdata( $post_id ) {
	global $post, $skudo_new_meta_boxes;
	
	if ( $the_post = wp_is_post_revision($post_id) ) $post_id = $the_post;

	if(get_post($post_id)->post_type=='page'){
		$skudo_new_meta_boxes=$GLOBALS['skudo_new_meta_boxes'];
		skudo_save_meta_data($skudo_new_meta_boxes, $post_id);
	}
}
 ...
 */
function skudo_save_portfolio_postdata( $post_id ) {
	global $post, $skudo_new_meta_portfolio_boxes;
	
	if ( $the_post = wp_is_post_revision($post_id) ) $post_id = $the_post;
	
	if(get_post($post_id)->post_type==SKUDO_PORTFOLIO_POST_TYPE){
		skudo_save_meta_data($skudo_new_meta_portfolio_boxes, $post_id);
	}
}
 ...
 * @param $post_id the ID of the post that contains the meta box
 */
function skudo_save_testimonials_postdata( $post_id ) {
	global $post, $skudo_new_meta_testimonials_boxes;
	
	if ( $the_post = wp_is_post_revision($post_id) ) $post_id = $the_post;
	
	if(get_post($post_id)->post_type==SKUDO_TESTIMONIALS_POST_TYPE){
		skudo_save_meta_data($skudo_new_meta_testimonials_boxes, $post_id);
	}
}
 ...
 * @param $post_id the ID of the post that contains the meta box
 */
function skudo_save_partners_postdata( $post_id ) {
	global $post, $skudo_new_meta_partners_boxes;
	
	if ( $the_post = wp_is_post_revision($post_id) ) $post_id = $the_post;
	
	if(get_post($post_id)->post_type==SKUDO_PARTNERS_POST_TYPE){
		skudo_save_meta_data($skudo_new_meta_partners_boxes, $post_id);
	}
}
 ...
 * @param $post_id the ID of the post that contains the meta box
 */
function skudo_save_team_postdata( $post_id ) {
	global $post, $skudo_new_meta_team_boxes;
 ...
 * @param $post_id the ID of the post that contains the meta box
 */
function skudo_save_post_postdata( $post_id ) {
	global $post, $skudo_new_meta_post_boxes;
	
	if ( $the_post = wp_is_post_revision($post_id) ) $post_id = $the_post;

	if(get_post($post_id)->post_type=='post'){
		skudo_save_meta_data($skudo_new_meta_post_boxes, $post_id);
	}
}

/**
 * Saves the post meta for all types of posts.
 * @param $skudo_new_meta_boxes the meta data array
 * @param $post_id the ID of the post
 */
function skudo_save_meta_data($skudo_new_meta_boxes, $post_id){

	if (isset($skudo_new_meta_boxes) && !empty($skudo_new_meta_boxes)){
			foreach($skudo_new_meta_boxes as $meta_box) {

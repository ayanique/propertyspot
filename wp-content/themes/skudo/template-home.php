...
get_header(); if (function_exists('skudo_print_menu')) skudo_print_menu(false);
	...
	switch($homeType){
		case "slider":
			if (get_post_meta($this_page_id, 'parallaxEffect_value', true) == 'yes') skudo_print_slider($this_page_id, true);
			else skudo_print_slider($this_page_id);
		break;
				...
				<?php
					if ($homeType == "image"){
						...
						<div id="parallax-home" <?php if (get_post_meta($this_page_id, 'parallaxEffect_value', true) == 'yes') echo 'class="parallax" data-stellar-background-ratio="0.5" '; ?> >
							...
							<?php 
								skudo_print_intro($this_page_id); 
								$skudo_output = "#home #parallax-home{background-image: url(".esc_url($media[1]).");background-size:cover;text-align:center;}";

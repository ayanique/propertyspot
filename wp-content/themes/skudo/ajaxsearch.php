...
function upper_search_ajax(){
	...
	if (!wp_verify_nonce($_POST['security'],'skudo-theme-search')) return;
	...
	if ( $skudo_the_query->have_posts() ) {
		...
		while ( $skudo_the_query->have_posts() ) {
			...
			if (get_option("skudo_search_show_author") == "on") {
				if (function_exists('icl_t')){
					$results .=", ".sprintf(esc_html__("%s","skudo"), icl_t( 'skudo', 'by', get_option('skudo_by_text')))." ".get_the_author();
				} else {
					$results .=", ".sprintf(esc_html__("%s","skudo"), get_option("skudo_by_text"))." ".get_the_author();
				}
			}
			if (get_option("skudo_search_show_date") == "on")
			$results .= ", ".get_the_date();
			if (get_option("skudo_search_show_categories") == "on"){
				...
				if ($categories){
					foreach($categories as $category) {
						if ($category->term_id != 1){
							if ($firstcat){
								if (function_exists('icl_t')){
									$results .= ", ".sprintf(esc_html__("%s","skudo"), icl_t( 'skudo', 'in', get_option('skudo_in_text')))." <i>";
								} else {
									$results .= ", ".sprintf(esc_html__("%s","skudo"), get_option("skudo_in_text"))." <i>";
								}
								$firstcat = false;
			...
			if (get_option("skudo_search_show_tags") == "on"){
				$tags = get_the_tags();
				$firsttag = true;
				if ($tags){
					foreach($tags as $tag) {
		...
		if (function_exists('icl_t')){
			$results .= "<li><a>".sprintf(esc_html__("%s","skudo"), icl_t( 'skudo', 'No results found.', get_option('skudo_no_results_text')))."</a></li>";
		} else {
			$results .= "<li><a>".sprintf(esc_html__("%s","skudo"), get_option("skudo_no_results_text"))."</a></li>";
		}
	}

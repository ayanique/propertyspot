 ...
 */
	...
	add_action( 'after_setup_theme', 'skudo_setup' );
	
	//body class
	function skudo_custom_body_class($classes, $class){
		if (is_singular() && get_post_meta(get_the_ID(), 'skudo_enable_custom_header_options_value', true)=='yes'){
			if (get_post_meta(get_the_ID(), 'skudo_content_to_the_top_value', true) == "off") $classes[] = "content_after_header";
		}
		else {
			if (get_option('skudo_content_to_the_top') == "off") $classes[] = "content_after_header";
		}
		return $classes;
	}
	...
	add_filter( 'body_class', 'skudo_custom_body_class', 10, 2 );
	
	//under construction feature.
	function skudo_under_construction(){
		$skudo_uc_id = get_option('skudo_under_construction_page');
		require_once(get_template_directory().'/template-under-construction.php');
		exit;
	}
	
	function skudo_setup(){
		
		//remove notifications
		add_action( 'vc_before_init', 'skudo_vcSetAsTheme' );
		function skudo_vcSetAsTheme() {
		    vc_set_as_theme(true);
			update_option('wpb_js_gutenberg_disable',1);
		}
		if (function_exists( 'set_revslider_as_theme' )){
			add_action( 'init', 'skudo_set_revslider_as_theme' );
			function skudo_set_revslider_as_theme() {
				set_revslider_as_theme();
				$rev_opts = get_option('revslider-global-settings');
				if ($rev_opts){
					$rev_opts = json_decode($rev_opts);
					if (is_object($rev_opts)){
						$rev_opts->include = "true";
						update_option('revslider-global-settings', json_encode($rev_opts));
					}
				} else {
					$rev_opts = new stdClass();
					$rev_opts->include = "true";
					update_option('revslider-global-settings', json_encode($rev_opts));
				}
			}
		 ...
		 */
		load_theme_textdomain( 'skudo', get_template_directory() . '/languages' );
		...
		global $skudo_new_meta_boxes,
		$skudo_new_meta_post_boxes,
		$skudo_new_meta_portfolio_boxes,
		$skudo_buttons,
		$skudo_data;
		$skudo_new_meta_boxes=array();
		$skudo_new_meta_post_boxes=array();
		$skudo_new_meta_portfolio_boxes=array();
		$skudo_buttons=array();
		$skudo_data=new stdClass();
		...
		define("SKUDO_VERSION", $my_theme->Version);
		//define the main paths and URLs
		define("SKUDO_LIB_PATH", get_template_directory() . '/lib/');
		define("SKUDO_LIB_URL", get_template_directory_uri().'/lib/');
		define("SKUDO_JS_PATH", get_template_directory_uri().'/js/');
		define("SKUDO_CSS_PATH", get_template_directory_uri().'/css/');
	
		define("SKUDO_FUNCTIONS_PATH", SKUDO_LIB_PATH . 'functions/');
		define("SKUDO_FUNCTIONS_URL", SKUDO_LIB_URL.'functions/');
		define("SKUDO_CLASSES_PATH", SKUDO_LIB_PATH.'classes/');
		define("SKUDO_OPTIONS_PATH", SKUDO_LIB_PATH.'options/');
		define("SKUDO_WIDGETS_PATH", SKUDO_LIB_PATH.'widgets/');
		define("SKUDO_SHORTCODES_PATH", SKUDO_LIB_PATH.'shortcodes/');
		define("SKUDO_PLUGINS_PATH", SKUDO_LIB_PATH.'plugins/');
		define("SKUDO_UTILS_URL", SKUDO_LIB_URL.'utils/');
		
		define("SKUDO_IMAGES_URL", SKUDO_LIB_URL.'images/');
		define("SKUDO_CSS_URL", SKUDO_LIB_URL.'css/');
		define("SKUDO_SCRIPT_URL", SKUDO_LIB_URL.'script/');
		define("SKUDO_PATTERNS_URL", get_template_directory_uri().'/images/skudo_patterns/');
		$uploadsdir=wp_upload_dir();
		define("SKUDO_UPLOADS_URL", $uploadsdir['url']);
		define("SKUDO_SEPARATOR", '|*|');
		define("SKUDO_OPTIONS_PAGE", 'skudo_options');
		define("SKUDO_STYLE_OPTIONS_PAGE", 'skudo_style_options');
		define("SKUDO_DEMOS_PAGE", 'skudo_demos');
	
		/*----------------------------------------------------------------
		 *  INCLUDE THE FUNCTIONS FILES
		 *---------------------------------------------------------------*/
				
		require_once (SKUDO_FUNCTIONS_PATH.'general.php');  //some main common functions
		require_once (SKUDO_FUNCTIONS_PATH.'stylesheet.php');  //some main common functions
		add_action('wp_enqueue_scripts', 'skudo_style', 1);
		add_action('wp_enqueue_scripts', 'skudo_scripts', 10);
		
		require_once (SKUDO_FUNCTIONS_PATH.'sidebars.php');  //the sidebar functionality
		if ( isset($_GET['page']) && $_GET['page'] == SKUDO_OPTIONS_PAGE ){
			require_once (SKUDO_CLASSES_PATH.'upper-options-manager.php');  //the theme options manager functionality
		}
		if ( isset($_GET['page']) && $_GET['page'] == SKUDO_STYLE_OPTIONS_PAGE ){
			require_once (SKUDO_CLASSES_PATH.'upper-style-options-manager.php');  //the theme options manager functionality
		}
		if ( isset($_GET['page']) && $_GET['page'] == SKUDO_DEMOS_PAGE ){
			require_once (SKUDO_CLASSES_PATH.'upper-demos-manager.php');  //the theme options manager functionality
		}
			
		require_once (SKUDO_CLASSES_PATH.'upper-templater.php');  
		require_once (SKUDO_CLASSES_PATH.'upper-custom-data-manager.php');  
		require_once (SKUDO_CLASSES_PATH.'upper-custom-page.php');  
		require_once (SKUDO_CLASSES_PATH.'upper-custom-page-manager.php');  
		require_once (SKUDO_FUNCTIONS_PATH.'custom-pages.php');  //the comments functionality
		require_once (SKUDO_FUNCTIONS_PATH.'comments.php');  //the comments functionality
		
		do_action( 'skudo_plugin_widgets_init' ); // the skudo-plugin-widgets functionality
		
		require_once (SKUDO_FUNCTIONS_PATH.'options.php');  //the theme options functionality
		
		if (is_admin()){
			require_once (SKUDO_FUNCTIONS_PATH. 'meta.php');  //adds the custom meta fields to the posts and pages
			add_action('admin_enqueue_scripts','skudo_admin_style');
		}
		$functions_path = get_template_directory() . '/functions/';
		
		add_filter('add_to_cart_fragments' , 'skudo_woocommerce_header_add_to_cart_fragment' );
		
		// Declare sidebar widget zone
		if (function_exists('register_sidebar')) {
			register_sidebar(array(
				'name' => esc_html__( 'Blog Sidebar', 'skudo' ),
				'id'   => 'sidebar-widgets',
		...
		if (!function_exists('skudo_wp_pagenavi')){ 
		...
		if ( function_exists( 'add_image_size' ) ) {
			add_image_size( 'skudo_blog', 1000, 563, true );				// Standard Blog Image
			add_image_size( 'skudo_mini', 80, 80, true ); 				// used for widget thumbnail
			add_image_size( 'skudo_portfolio', 600, 400, true );			// also for blog-medium
			add_image_size( 'skudo_regular', 500, 500, true ); 
			add_image_size( 'skudo_wide', 1000, 500, true ); 
			add_image_size( 'skudo_tall', 500, 1000, true );
			add_image_size( 'skudo_widetall', 1000, 1000, true ); 
		}
		...
		add_action( 'tgmpa_register', 'skudo_register_required_plugins' );	
		...
		if (get_option("skudo_enable_smooth_scroll") == "on"){
			update_option('ultimate_smooth_scroll','enable');
		} else update_option('ultimate_smooth_scroll','disable');
		add_filter( 'woocommerce_enqueue_styles', '__return_false' );
	}
	
	function skudo_admin_style(){
		wp_enqueue_style('skudo-font-awesome-painel-custom', SKUDO_CSS_PATH .'font-awesome-painel-custom.min.css');
		wp_enqueue_script( 'skudo-admin', SKUDO_JS_PATH .'skudo-admin.js', array(), '1',$in_footer = true);
	}
	
	
	
	function skudo_wpml_filter_langs( $languages ) {
	...
	add_filter( 'icl_ls_languages', 'skudo_wpml_filter_langs' );
	add_filter('wpml_add_language_selector', 'skudo_wpml_filter_langs');
	...
	function skudo_style() {
		$theme = wp_get_theme();
		wp_enqueue_style( 'skudo-style', get_template_directory_uri().'/style.css', array(), $theme->Version );
	}
	
	function skudo_slug_post_classes( $classes, $class, $post_id ) {
		$skudo_is_portfolio = array_search( 'type-portfolio', $classes );
		if ( is_single( $post_id ) && false !== $skudo_is_portfolio ) {
			$classes[] = 'container';
		}
		if (is_sticky( $post_id )) $classes[] = 'sticky';	 
		return $classes;
	}
	add_filter( 'post_class', 'skudo_slug_post_classes', 10, 3 );
	...
	/*-----------------------------------------------------------------------------------*/
	function skudo_autoimport($url, $demo) {
	...
	function skudo_get_string_between($string, $start, $end){
	    $string = " ".$string;
	...
	add_action('init', 'skudo_allowed_tags', 10);

	function skudo_get_the_woo(){
		global $woocommerce;
	...
	function skudo_google_fonts_scripts() {
	    wp_enqueue_style( 'skudo-google-fonts', skudo_fonts_url(), '' );
	}
	
	function skudo_get_custom_inline_css(){
		global $skudo_inline_css;
		wp_enqueue_style('skudo-custom-style', SKUDO_CSS_PATH .'skudo-custom.css',99);
		if (!$skudo_inline_css) $skudo_inline_css = "";
		$skudo_inline_css .= "body{visibility:visible;}";
		wp_add_inline_style('skudo-custom-style', $skudo_inline_css);
	}
	
	function skudo_set_custom_inline_css($css){
		global $skudo_inline_css;
		$upper_theme_main_color = "#".get_option('skudo_style_color');
		$skudo_inline_css .= str_replace( '__USE_THEME_MAIN_COLOR__', $upper_theme_main_color, $css );
	}
	
	function skudo_set_team_profiles_content($content){
		global $skudo_team_profiles;
		if (!isset($skudo_team_profiles)) $skudo_team_profiles = '';
		$skudo_team_profiles .= $content;
	}
	
	function skudo_get_team_profiles_content(){
		global $skudo_team_profiles;
		if (isset($skudo_team_profiles)){
			$skudo_team_profiles = wp_kses_no_null( $skudo_team_profiles, array( 'slash_zero' => 'keep' ) );
			$skudo_team_profiles = wp_kses_normalize_entities($skudo_team_profiles);
			echo wp_kses_hook($skudo_team_profiles, 'post', array());
		}
	}
	...
	// ajax workers
	//front
	if (get_option('skudo_enable_search')=="on" && get_option('skudo_enable_ajax_search')=="on"){
	...
	if ( is_admin() && current_user_can( 'manage_options' ) ) {
		...
		require_once get_template_directory() . '/lib/functions/skudo_demo_installer.php';
		require_once get_template_directory() . '/lib/utils/upload-handler.php';
		$skudo_imported_ids = get_option('skudo_imported_ids');
		if (!$skudo_imported_ids){
			$skudo_imported_ids = array();
			update_option('skudo_imported_ids', $skudo_imported_ids);
		}
		$skudo_imported_ids;
	}

	if (!function_exists('skudo_get_imported_ids')){
		function skudo_get_imported_ids(){
			global $skudo_imported_ids;
			return $skudo_imported_ids;
		}
	}

	if (!function_exists('skudo_set_imported_ids')){
		function skudo_set_imported_ids($ids){
			global $skudo_imported_ids;
			$skudo_imported_ids = $ids;
			update_option('skudo_imported_ids', $skudo_imported_ids);
		}
	}

	if (!function_exists('skudo_add_imported_id')){
		function skudo_add_imported_id( $id, $revs = false ){
			$aux = get_option('skudo_imported_ids');
			$output = "";
			if ($revs){
				if ( !array_search( $id, $aux ) ){
					array_push( $aux, $id );
					skudo_set_imported_ids($aux);
					return true;
				} else {
					return false;
				}

...
function initBlogs() {
            ...
            start: function(e) {
                jQuery(e).find(".flex-direction-nav li a").each(function() {
                    jQuery(this).on("mouseenter", function() {
                        jQuery(this).css("background-color", window.skudoOptions.styleColor)
    ...
    }), jQuery(".container .vendor").length && jQuery(".container .vendor").fitVids(), jQuery(window).resize()
}

function skudo_monitorScrollTop() {
...
jQuery(document).ready(function(e) {
    ...
    "use strict";
    window.skudo_pageNum = 0 != parseInt(window.skudoOptions.loader_startPage, 10) ? parseInt(window.skudoOptions.loader_startPage, 10) : 1, window.skudo_totalForward = 1, window.skudo_totalBackward = -1;
    var t = parseInt(window.skudoOptions.loader_maxPages),
        r = "empty";
    jQuery(".navigation .next-posts").parent().attr("href") && (r = jQuery(".navigation .next-posts").parent().attr("href"));
    var a = "empty";
    jQuery(".navigation .prev-posts").parent().attr("href") && (a = jQuery(".navigation .prev-posts").parent().attr("href")), "empty" == r && "empty" == a || "scroll" !== window.skudoOptions.reading_option && "scrollauto" !== window.skudoOptions.reading_option || (parseInt(window.skudo_pageNum, 10) + 1 <= t && jQuery(".post-listing").each(function() {
        if (0 == jQuery(this).parents(".recentPosts_widget").length && 0 == jQuery(this).parents(".single").length) {
            var e = '<p id="pbd-alp-load-posts"><a href="javascript:;">' + window.skudoOptions.skudo_load_more_posts_text + "</a></p>";
            jQuery(this).parent().append('<div style="position:relative;float:left;display:inline-block;width:100%;" class="pbd-alp-placeholder pbd-alp-placeholder-' + (parseInt(window.skudo_pageNum, 10) + 1) + '"></div>').append(e)
        }
        0 == jQuery(this).parents(".single").length && jQuery(".navigation").css("display", "none")
    }), parseInt(window.skudo_pageNum, 10) > 1 && jQuery(".post-listing").each(function() {
        if (0 == jQuery(this).parents(".recentPosts_widget").length && 0 == jQuery(this).parents(".single").length) {
            var e = '<p id="pbd-alp-load-newer-posts" style="position: relative; height:50px;width:100%;text-align:center;"><a style=" position: relative; display:inline-block;" href="javascript:;">' + window.skudoOptions.skudo_load_more_posts_text + "</a></p>";
            jQuery(this).parent().prepend('<div class="pbd-alp-placeholder-newer-' + (parseInt(window.skudo_pageNum, 10) - 1) + '"></div>').prepend(e)
        }
        0 == jQuery(this).parents(".single").length && jQuery(".navigation").css("display", "none")
    }), jQuery("#pbd-alp-load-posts a").length && jQuery("#pbd-alp-load-posts a").on("click", function(e) {
        return window.skudo_pageNum + window.skudo_totalForward <= t ? (jQuery(this).html(window.skudoOptions.skudo_loading_posts_text + '<img style="width:16px; height: 16px; margin-left: 5px; position: relative;" src="' + window.skudoOptions.templatepath + '/images/ajx_loading.gif">'), jQuery(".pbd-alp-placeholder-" + parseInt(window.skudo_pageNum + window.skudo_totalForward, 10)).load(r + " article", function() {
            if (jQuery(".pbd-alp-placeholder-" + parseInt(window.skudo_pageNum + window.skudo_totalForward, 10) + " article").each(function() {
        ...
        }).fadeIn(5e3)) : window.clearInterval(window.skudo_interval), !1
    }), jQuery("#pbd-alp-load-newer-posts a").length && jQuery("#pbd-alp-load-newer-posts a").on("click", function(e) {
        return window.skudo_pageNum + window.skudo_totalBackward > 0 ? (jQuery(this).html(window.skudoOptions.skudo_loading_posts_text + '<img style="width:16px; height: 16px; margin-left: 5px; position: relative;" src="' + window.skudoOptions.templatepath + '/images/ajx_loading.gif">'), jQuery(".pbd-alp-placeholder-newer-" + (window.skudo_pageNum + window.skudo_totalBackward)).load(a + " article", function() {
            if (jQuery(".pbd-alp-placeholder-newer-" + (window.skudo_pageNum + window.skudo_totalBackward) + " article").each(function() {
                ...
                }), window.skudo_totalBackward = window.skudo_totalBackward - 1, a = a.indexOf("paged") < 0 ? a.replace(/\/page\/[0-9]?/, "/page/" + (window.skudo_pageNum + window.skudo_totalBackward)) : a.replace(/paged=[0-9]?/, "paged=" + (window.skudo_pageNum + window.skudo_totalBackward)), jQuery("#pbd-alp-load-newer-posts").after('<div class="pbd-alp-placeholder-newer-' + parseInt(window.skudo_pageNum + window.skudo_totalBackward) + '"></div>'), window.skudo_pageNum + window.skudo_totalBackward + 1 > 1) jQuery("#pbd-alp-load-newer-posts a").text(window.skudoOptions.skudo_load_more_posts_text);
            else {
                jQuery("#pbd-alp-load-newer-posts a").text(window.skudoOptions.skudo_no_more_posts_text);
                setTimeout(function() {
                    jQuery("#pbd-alp-load-newer-posts").slideUp(500).fadeOut(500)
                }, 5e3)
            }
            jQuery(".post-listing").hasClass("journal") && jQuery(".pbd-alp-placeholder-newer-" + (window.skudo_pageNum + window.skudo_totalBackward)).remove(), initBlogs()
        }).fadeIn(5e3)) : window.clearInterval(window.skudo_interval), !1
    }))
});
...
if (!class_exists("ezTweet")){
	class ezTweet {
		...
		public function __construct() {
			...
			$this->lib = $_POST['templatepath'].'wp-content/plugins/skudo_custom_post_types/lib/';
		...
		private function getTwitterJSON() {
			...
			if (!isset($_POST['request'])){
				$_POST['request'] = array(
					...
					"parameters" => array(
						"include_entities" => 1,
						"screen_name" => array(get_option('skudo_twitter_username')),
						"page" => 1,
						"count" => intval(get_option('skudo_twitter_number_tweets')),
						"include_rts" => 1
					)
				);
			}
...
function upper_ajax_twitter(){
	...
	if (!wp_verify_nonce($_POST['security'],'skudo-theme-twitter')) return;
	
	include_once $_POST['templatepath'] . 'wp-content/plugins/skudo_custom_post_types/lib/tmhOAuth.php';

<?php
	...
	function skudo_print_intro($id, $video = false){
			...
			if ($type !== "none"){
				...
				<div class="home-logo-<?php echo esc_attr($type); ?> home-logo">
					<?php
						switch($type){
							case "text":
								$font = get_post_meta($id,'introLogoFont_value',true); $skudo_import_fonts[] = $font; $font = explode("|",$font); if ($font[0] == "Helvetica" || $font[0] == "Helvetica Neue") $font[0] = $font[0]."', 'Arial', 'sans-serif"; if (!isset($font[1])) $font[1] = "";
								...
								<?php
								$skudo_output = ".home-logo a.nav-to{font-family:".esc_attr($font[0]).";font-weight:".esc_attr($font[1]).";font-size:".(int) preg_replace('/\D/', '', $fontsize)."px;";
								if ($border === "yes") $skudo_output.= "padding: 5px 25px 5px;border: 3px solid;letter-spacing: 3px;";
								$skudo_output .= "}";
							...
							case "image":
								...
								<a href="<?php if ($link === "") $link = "#"; echo esc_url($link); ?>" class="nav-to">
									<img src="<?php echo esc_url($imageurl[1]); ?>" alt="<?php echo esc_attr(get_the_title()); ?>" />
								</a>
								<?php
								$skudo_output = ".home-logo a img{height:".(int) preg_replace('/\D/', '', $height)."px;}";
								skudo_set_custom_inline_css($skudo_output);
							break;
						}
					?>
				...
				<div id="home-slider" class="flexslider">
					<ul class="slides styled-list">
						<?php
							...
							foreach($slides as $s){
								if ($s != ""){
									$font = get_post_meta($id,'introCaptionsFont_value',true); $skudo_import_fonts[] = $font; $font = explode("|",$font); if ($font[0] == "Helvetica" || $font[0] == "Helvetica Neue") $font[0] = $font[0]."', 'Arial', 'sans-serif"; if (!isset($font[1])) $font[1] = "";
									?>
									<li class="home-slide">
							...
							$skudo_output = "#home-slider.flexslider li p.home-slide-content{font-family:'".esc_attr($font[0])."';font-weight:".esc_attr($font[1]).";color:#".esc_attr(get_post_meta($id,'introCaptionsTextStyle_value',true)).";}";
							skudo_set_custom_inline_css($skudo_output);
						?>
					</ul>
				</div>
				...
				<div class="intro_continue intro_continue_<?php echo esc_attr(get_post_meta($id, 'introContinueType_value', true)); ?>">
					...
					<?php
						if (get_post_meta($id, 'introContinueType_value', true) === "text"){
							$font = get_post_meta($id,'introContinueFont_value',true); $skudo_import_fonts[] = $font; $font = explode("|",$font); if ($font[0] == "Helvetica" || $font[0] == "Helvetica Neue") $font[0] = "'".$font[0]."', 'Arial', 'sans-serif"; if (!isset($font[1])) $font[1] = "";
							...
							$skudo_output = ".intro_continue p{font-family:".esc_attr($font[0]).";font-weight:".$font[1].";font-size:".esc_attr($fontsize)."px;color:#".esc_attr($fontcolor)."background:#".esc_attr($fontbgcolor).";}";
							skudo_set_custom_inline_css($skudo_output);
						} else {
							?>
							<img src="<?php echo esc_url(get_template_directory_uri()); ?>/images/next-section.png" alt="continue">
							<?php
						}
					?>
					</a>

...
function skudo_print_menu($ispagephp = true, $isfirstpage = false){
	...
	wp_enqueue_script( 'upper-header-style6', get_template_directory_uri().'/js/utils/upper-header-style6.js', array('jquery'), 1, true );
	...
	<header class="header-init navbar navbar-default navbar-fixed-top <?php echo esc_attr($typeofheader . $skudo_underlining_effect); ?> <?php if (get_option('skudo_fixed_menu') == 'off') echo " header_not_fixed"; else if (get_option('skudo_header_hide_on_start') == "on" && !$ispagephp) echo " hide-on-start"; ?><?php if (get_option("skudo_header_full_width") == "on") echo " header-full-width"; ?><?php if (get_option("skudo_header_full_width") == "off") echo " header-with-container"; ?><?php if (get_option("skudo_header_menu_itens_style") == "rounded") echo " menu-rounded"; ?><?php if (get_option("skudo_header_menu_itens_style") == "simple") echo " menu-simple"; ?><?php if (get_option("skudo_header_menu_itens_style") == "square") echo " menu-square"; ?><?php echo " ".esc_attr($skudo_header_bgstyle_pre); ?><?php if ($skudo_content_at_top) echo " header-init-force-fixed "; ?>" data-rel="<?php echo esc_attr($skudo_header_bgstyle_pre."|".$skudo_header_bgstyle_after); ?>">
		...
		if (get_option("skudo_info_above_menu") == "on"){
			...
			<div class="top-bar">
				<div class="top-bar-bg">
					<div class="<?php if (get_option("skudo_header_full_width") == "off") echo "container"; ?> clearfix">
						<div class="slidedown">
								...
								if ( get_option("skudo_telephone_menu") != "" || get_option("skudo_email_menu") != "" || get_option("skudo_address_menu") != "" || get_option("skudo_text_field_menu") != "" ){
									...
									<ul class="phone-mail">
										...
										<?php if ( is_string(get_option("skudo_text_field_menu")) && get_option("skudo_text_field_menu") != "" ){ ?>
											<li class="text_field"><?php echo wp_kses_post(get_option("skudo_text_field_menu")); ?></li>
										<?php } ?>
									</ul>
									<?php
								}
		...
		<div class="nav-container <?php if (get_option("skudo_header_full_width") == "off") echo " container"; ?>">
			...
			<div class="navbar-collapse collapse">
				<?php 
					if (get_option('skudo_header_button') == 'on'){
						?>
						<div class="skudo-header-button">
							<a href="<?php echo esc_url(get_option('skudo_header_button_link')); ?>" class="nav-to"><?php 
								if (function_exists('icl_t')){
									printf(esc_html__("%s","skudo"), icl_t( 'skudo', 'Header Button', get_option('skudo_header_button_text')));
								} else {
									printf(esc_html__("%s","skudo"), get_option("skudo_header_button_text"));
								} ?>
							</a>
						</div>
						<?php
	...
	<nav class="nav-style-6">
		<div class="nav-style-6">
			<?php
				if (!$isfirstpage){
					if ($ispagephp){
						wp_nav_menu( array( 'theme_location' => 'PrimaryNavigation', 'container' => false, 'menu_class' => 'nav navbar-nav navbar-right cd-primary-nav', 'walker' => new skudo_walker_nav_menu_outsider, 'fallback_cb' => esc_html__('You need to assign a Menu to the Main Navigation Location.','skudo') ) );
					} 
					else {
						global $homes;
						$homes = 0;
						wp_nav_menu( array( 'theme_location' => 'PrimaryNavigation', 'container' => false, 'menu_class' => 'nav navbar-nav navbar-right cd-primary-nav', 'walker' => new skudo_walker_nav_menu, 'fallback_cb' => esc_html__('You need to assign a Menu to the Main Navigation Location.','skudo') ) );
					}
				}
			?>
		</div>

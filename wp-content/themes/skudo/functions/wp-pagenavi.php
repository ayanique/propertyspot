...
*/
function skudo_pagenavi_textdomain() {
	load_plugin_textdomain('wp-pagenavi', false, 'wp-pagenavi');
}

function skudo_wp_pagenavi($before = '', $after = '') {
	global $wpdb, $wp_query, $skudo_the_query;
	...
	if (!is_single()) {
		...
		if (!empty($skudo_the_query)){
			$wp_query = $skudo_the_query;
		}
		...
		if (empty($skudo_the_query)) $max_page = ceil($totalPosts / $posts_per_page);
		...
		$larger_start_page_start = (skudo_n_round($start_page, 10) + $larger_page_multiple) - $larger_per_page;
		$larger_start_page_end = skudo_n_round($start_page, 10) + $larger_page_multiple;
		$larger_end_page_start = skudo_n_round($end_page, 10) + $larger_page_multiple;
		$larger_end_page_end = skudo_n_round($end_page, 10) + ($larger_per_page);
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			$pages_text = str_replace("_CURRENT_PAGE_", $paged, esc_html__('Page _CURRENT_PAGE_ of _TOTAL_PAGES_','skudo'));
			$pages_text = str_replace("_TOTAL_PAGES_", $max_page, $pages_text);
			$pages_text = esc_html__("Page ","skudo") . $paged . esc_html__(" of ","skudo") . $max_page;
			echo '<div class="des-pages">'."\n";
			
			switch(get_option("skudo_blog_reading_type")){
			...
			switch(intval($pagenavi_options['style'])) {
				...
				case 2:
					...
					echo '<label class="gotopagedd" for="pagedd">'. esc_html__('Go to page','skudo') .'</label>';
...
function skudo_n_round($num, $tonearest) {
   return floor($num/$tonearest)*$tonearest;
}

### Function: Page Navigation Options
function skudo_pagenavi_init() {
	skudo_pagenavi_textdomain();
	// Add Options
	$pagenavi_options = array();
	$pagenavi_options['pages_text'] = esc_html__('Page _CURRENT_PAGE_ of _TOTAL_PAGES_','skudo');
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = esc_html__('&laquo;','skudo');
	$pagenavi_options['last_text'] = esc_html__('&raquo;','skudo');
	$pagenavi_options['next_text'] = esc_html__('&raquo;','skudo');
	$pagenavi_options['prev_text'] = esc_html__('&laquo;','skudo');
	$pagenavi_options['dotright_text'] = esc_html__('...','skudo');
	$pagenavi_options['dotleft_text'] = esc_html__('...','skudo');

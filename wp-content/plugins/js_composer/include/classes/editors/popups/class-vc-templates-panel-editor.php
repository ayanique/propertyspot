...
class Vc_Templates_Panel_Editor {
	...
	public function getAllTemplates() {
		...
		if ( ! empty( $default_templates ) ) {
			$arr_category = array(
				...
				'category_name' => __( 'Skudo Templates', 'js_composer' ),
				'category_description' => __( 'Append predefined Upper templates to the current layout', 'js_composer' ),
				'category_weight' => 10,
				/*UPPER addition end*/
			);
			$category_templates = array();
	...
	public function renderTemplateListItem( $template ) {
		...
		$custom_class = esc_attr( isset( $template['custom_class'] ) ? $template['custom_class'] : '' );
				...
				/*UPPER addition - adding in preview img*/

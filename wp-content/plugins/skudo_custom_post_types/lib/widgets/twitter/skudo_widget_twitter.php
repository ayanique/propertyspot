...
class Skudo_Twitter_Widget extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'twitter_widget', 'description' => esc_html__('Show your tweets on your site.', 'skudo'));
...
?>  
      ...
      <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>">&#8212; <?php esc_html_e('Title','skudo'); ?> &#8212;<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_html($title); ?>" /></label></p> 
       <p><label for="<?php echo esc_attr($this->get_field_id('ntweets')); ?>">&#8212; <?php esc_html_e('Number Tweets to show','skudo'); ?> &#8212;<input class="widefat" id="<?php echo esc_attr($this->get_field_id('ntweets')); ?>" name="<?php echo esc_attr($this->get_field_name('ntweets')); ?>" type="text" value="<?php echo esc_attr($ntweets); ?>" /><br><span class="flickr-stuff">If 0 will show the last tweet.</span></label></p>
...
function widget($args, $instance) {
	...
	if ( (get_option("skudo_twitter_username") == "") || (get_option("twitter_consumer_key") == "") || (get_option("twitter_consumer_secret") == "") || (get_option("twitter_user_token") == "") || (get_option("twitter_user_secret") == "") || (get_option("skudo_twitter_number_tweets") == "") ){
		$twitter_data_is_filled = false;
	}
    if ($twitter_data_is_filled) wp_enqueue_script( 'skudo-tweet', SKUDO_JS_PATH .'twitter/jquery.tweet.js', array(),'1.0',$in_footer = true);
    ?>
			 ...
			 <?php
				 if ($twitter_data_is_filled){
					...
					$skudo_inline_script = '
				        ...
				        jQuery.fn.extend({
				            ...
				            linkHash: replacer(/(?:^| )[\#]+([\w\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff\u0600-\u06ff]+)/gi,
				                " <a href=\"https://twitter.com/search?q=%23$1"+(("'.(is_string(get_option("skudo_twitter_username"))?get_option("skudo_twitter_username"):'').'" && "'.(is_string(get_option("skudo_twitter_username"))?get_option("skudo_twitter_username"):'').'".length == 1 && !s.list) ? "&from="+s.username.join("%2BOR%2B") : "")+"\" class=\"tweet_hashtag\">#$1</a>"),
					...
					wp_add_inline_script('skudo-global', $skudo_inline_script, 'after');
					 ...
					 ?>
					 <p>Please fill the fields on the <strong>Appearance > Skudo Options > Twitter and Social Icons > Twitter</strong> panel with your credentials.</p>
					 <?php
				 }
			 ?>
			</div>
...
register_widget('Skudo_Twitter_Widget');

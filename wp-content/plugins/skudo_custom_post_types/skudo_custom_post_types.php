<?php
/*
Plugin Name: Skudo Custom Post Types
Plugin URI: http://upperthemes.com
Description: Testimonials, Partners, Team and Projects Posts + Widgets. We do not intended this plugin for distribution. We are only responsible for its usage with Skudo theme.
Version: 2.4
Author: Upper
...
if (!defined('SKUDO_PORTFOLIO_POST_TYPE')){
	if (!defined('SKUDO_SHORTNAME')) define('SKUDO_SHORTNAME', 'skudo');
	$portfolio_permalink = get_option(SKUDO_SHORTNAME."_portfolio_permalink");
	if (!get_option(SKUDO_SHORTNAME."_portfolio_permalink")) define("SKUDO_PORTFOLIO_POST_TYPE", "portfolio");
	else define("SKUDO_PORTFOLIO_POST_TYPE", get_option(SKUDO_SHORTNAME."_portfolio_permalink"));
}
if (!defined('SKUDO_TESTIMONIALS_POST_TYPE')){
	define("SKUDO_TESTIMONIALS_POST_TYPE", 'testimonials');
}
if (!defined('SKUDO_PARTNERS_POST_TYPE')){
	define("SKUDO_PARTNERS_POST_TYPE", 'partners');
}
if (!defined('SKUDO_TEAM_POST_TYPE')){
	define("SKUDO_TEAM_POST_TYPE", 'team');	
}
if (!defined('SKUDO_PLG_URL')){
	define('SKUDO_PLG_URL', plugin_dir_url(__FILE__) );
}
if (!defined('SKUDO_PLG_PATH')){
	define('SKUDO_PLG_PATH', plugin_dir_path(__FILE__) );
}
if (!defined('SKUDO_PLG_ACTIVE')){
	define('SKUDO_PLG_ACTIVE', true );
}


/* widgets */
add_action( 'skudo_plugin_widgets_init', 'skudo_plugin_widgets_init' );
function skudo_plugin_widgets_init(){
	global $vc_addons_url;
	$active_widgets = ["contactForm/skudo_widget_contactForm.php","contactInfo/skudo_widget_contactInfo.php","flickr/skudo_widget_flickr.php","instagram/skudo_widget_instagram.php","newsletter/skudo_widget_newsletter.php","recentComments/skudo_widget_recentComments.php", "recentPostsSidebar2/skudo_widget_recentPostsSidebar2.php","twitter/skudo_widget_twitter.php","vcelement/skudo_widget_vc_element.php","video/skudo_widget_video.php"];
	if (isset($vc_addons_url) && $vc_addons_url != ""){
		$active_widgets_with_addons = ["partners/skudo_widget_partners.php","recentPostsSidebar/skudo_widget_recentPostsSidebar.php","team/skudo_widget_team.php","testimonials/skudo_widget_testimonials.php"];
		$active_widgets = array_merge($active_widgets, $active_widgets_with_addons); 
	}
	if (!class_exists('CubePortfolioMain')) unset($active_widgets[ array_search("recentProjects/skudo_widget_recentProjects.php", $active_widgets) ]);
	foreach ($active_widgets as $widget) require_once( "lib/widgets/" . $widget );
}

/* gutenberg */ 
add_action( 'init', 'skudo_check_for_gutenberg', 1 );
if (!function_exists('skudo_check_for_gutenberg')){
	function skudo_check_for_gutenberg(){
		global $wp_version, $pagenow;
		
		$_wpb_vc_js_status = isset($_GET['post']) ? get_post_meta( $_GET['post'], '_wpb_vc_js_status', true ) : false;
		if (!$_wpb_vc_js_status){
			if (in_array( $pagenow, array( 'post-new.php' ) ) && function_exists('vc_remove_element') && get_option( 'wpb_js_gutenberg_disable', 1 ) )
				$_wpb_vc_js_status = true;
		}
		if ( ( defined('GUTENBERG_VERSION') || intval($wp_version) > 4 ) && !isset($_GET['classic-editor']) && !$_wpb_vc_js_status && function_exists('vc_remove_element') ){
...
if (!function_exists('upper_admin_block_style')){
	function upper_admin_block_style(){
		wp_enqueue_style( 'upper-block', SKUDO_PLG_URL . "lib/gutenberg/upper-block.css", 1 );
	}
}
 ...
 * ADD THE ACTIONS
 */
add_action('init', 'skudo_register_portfolio_category');  //functions/portfolio.php
add_action('init', 'skudo_register_portfolio_post_type');  //functions/portfolio.php
 ...
 */
if (!function_exists('skudo_register_portfolio_category')){
    function skudo_register_portfolio_category(){

        register_taxonomy("portfolio_category",
            array(SKUDO_PORTFOLIO_POST_TYPE),
 ...
 */
if (!function_exists('skudo_register_portfolio_post_type')){
    function skudo_register_portfolio_post_type() {
        $portfolio_permalink = get_option(SKUDO_SHORTNAME."_portfolio_permalink");
        //the labels that will be used for the portfolio items
        $labels = array(
            'name' => _x('Projects', 'portfolio name','skudo'),
            'singular_name' => _x('Project Item', 'portfolio type singular name','skudo'),
            'add_new' => __('Add New','skudo'),
            'add_new_item' => __('Add New Item','skudo'),
            'edit_item' => __('Edit Item','skudo'),
            'new_item' => __('New Project Item','skudo'),
            'view_item' => __('View Item','skudo'),
            'search_items' => __('Search Project Items','skudo'),
            'not_found' =>  __('No project items found','skudo'),
            'not_found_in_trash' => __('No project items found in Trash','skudo'),
            'parent_item_colon' => ''
        );

        //register the custom post type
        register_post_type( SKUDO_PORTFOLIO_POST_TYPE,
            array( 'labels' => $labels,
                'public' => true,
                'show_ui' => true,
                'capability_type' => 'post',
                'menu_icon' => get_template_directory_uri() . '/images/skudo_icons/projectsicon.png',
                'hierarchical' => false,
 ...
 */
if (!function_exists('skudo_get_taxonomies')){
    function skudo_get_taxonomies($type){
        $args = array(
            'type' => 'post',
            'orderby' => 'id',
            'order' => 'ASC',
 ...
 */
if (!function_exists('skudo_get_taxonomy_slug')){
    function skudo_get_taxonomy_slug($term_id){
        global $wpdb;
 ...
 */
if (!function_exists('skudo_get_taxonomy_children')){
    function skudo_get_taxonomy_children($type, $parent_id){
        global $wpdb;
...
if (!function_exists('skudo_get_projects')){
    function skudo_get_projects(){
        $proj = array();
        $args= array(
            'posts_per_page' =>-1,
            'post_type' => SKUDO_PORTFOLIO_POST_TYPE
        );
        query_posts($args);
 ...
 * ADD THE ACTIONS
 */
add_action('init', 'skudo_register_partners_post_type');  //functions/partners.php


/**
 * Registers the portfolio custom type.
 */
if (!function_exists('skudo_register_partners_post_type')){
    function skudo_register_partners_post_type() {

        register_taxonomy("partners_category",
            array(SKUDO_PARTNERS_POST_TYPE),
            array(	"hierarchical" => true,
                "label" => "Categories",
                "singular_label" => "Categories",
                "rewrite" => true,
        ...
        //the labels that will be used for the portfolio items
        $labels = array(
            'name' => _x('Partners', 'partners name','skudo'),
            'singular_name' => _x('Partners Item', 'partners type singular name','skudo'),
            'add_new' => __('Add New','skudo'),
            'add_new_item' => __('Add New Item','skudo'),
            'edit_item' => __('Edit Item','skudo'),
            'new_item' => __('New Partners Item','skudo'),
            'view_item' => __('View Item','skudo'),
            'search_items' => __('Search Partners Items','skudo'),
            'not_found' =>  __('No Partners items found','skudo'),
            'not_found_in_trash' => __('No partners items found in Trash','skudo'),
            'parent_item_colon' => ''
        );

        //register the custom post type
        register_post_type( SKUDO_PARTNERS_POST_TYPE,
            array( 'labels' => $labels,
                'public' => true,
                'show_ui' => true,
                'exclude_from_search' => true,
                'show_in_nav_menus' => false,
                'menu_icon' => get_template_directory_uri() . '/images/skudo_icons/partnersicon.png',
                'capability_type' => 'post',
                'hierarchical' => false,
 ...
 * ADD THE ACTIONS
 */
add_action('init', 'skudo_register_team_post_type');  //functions/team.php


/**
 * Registers the portfolio custom type.
 */
if (!function_exists('skudo_register_team_post_type')){
    function skudo_register_team_post_type() {

        register_taxonomy("team_category",
            array(SKUDO_TEAM_POST_TYPE),
            array(	"hierarchical" => true,
                "label" => "Categories",
                "singular_label" => "Categories",
                "rewrite" => true,
        ...
        //the labels that will be used for the portfolio items
        $labels = array(
            'name' => _x('Team', 'team name','skudo'),
            'singular_name' => _x('Team Item', 'team type singular name','skudo'),
            'add_new' => __('Add New','skudo'),
            'add_new_item' => __('Add New Item','skudo'),
            'edit_item' => __('Edit Item','skudo'),
            'new_item' => __('New Team Item','skudo'),
            'view_item' => __('View Item','skudo'),
            'search_items' => __('Search Team Items','skudo'),
            'not_found' =>  __('No Team items found','skudo'),
            'not_found_in_trash' => __('No team items found in Trash','skudo'),
            'parent_item_colon' => ''
        );

        //register the custom post type
        register_post_type( SKUDO_TEAM_POST_TYPE,
            array( 'labels' => $labels,
                'public' => true,
                'show_ui' => true,
                'exclude_from_search' => true,
                'show_in_nav_menus' => false,
                'menu_icon' => get_template_directory_uri() . '/images/skudo_icons/icon71.png',
                'capability_type' => 'post',
                'hierarchical' => false,
 ...
 * ADD THE ACTIONS
 */
add_action('init', 'skudo_register_testimonials_post_type');  //functions/testimonials.php


/**
 * Registers the portfolio custom type.
 */
if (!function_exists('skudo_register_testimonials_post_type')){
    function skudo_register_testimonials_post_type() {

        register_taxonomy("testimonials_category",
            array(SKUDO_TESTIMONIALS_POST_TYPE),
            array(	"hierarchical" => true,
                "label" => "Categories",
                "singular_label" => "Categories",
                "rewrite" => true,
        ...
        //the labels that will be used for the portfolio items
        $labels = array(
            'name' => _x('Testimonials', 'testimonials name','skudo'),
            'singular_name' => _x('Testimonials Item', 'testimonials type singular name','skudo'),
            'add_new' => __('Add New','skudo'),
            'add_new_item' => __('Add New Item','skudo'),
            'edit_item' => __('Edit Item','skudo'),
            'new_item' => __('New Testimonials Item','skudo'),
            'view_item' => __('View Item','skudo'),
            'search_items' => __('Search Testimonials Items','skudo'),
            'not_found' =>  __('No testimonials items found','skudo'),
            'not_found_in_trash' => __('No testimonials items found in Trash','skudo'),
            'parent_item_colon' => ''
        );

        //register the custom post type
        register_post_type( SKUDO_TESTIMONIALS_POST_TYPE,
            array( 'labels' => $labels,
                'public' => true,
                'show_ui' => true,
                'exclude_from_search' => true,
                'show_in_nav_menus' => false,
                'menu_icon' => get_template_directory_uri() . '/images/skudo_icons/testicon.png',
                'capability_type' => 'post',
                'hierarchical' => false,
...
if (!function_exists('skudo_init_cpt_plugin')){
    function skudo_init_cpt_plugin(){
        skudo_register_partners_post_type();
        skudo_register_team_post_type();
        skudo_register_testimonials_post_type();
    }
}


/*  EXTEND VC SHORTCODES  */
if (!function_exists('skudo_team_categories_settings_field')){
	function skudo_team_categories_settings_field($settings, $value){
		...
		if (!count($tax_terms)){
			...
			if (count($tax_terms) > 1) $output .= "<label class='team_categories'><input class='selectall' type='checkbox' name='categories[]' value='0' onchange=\"if(jQuery(this).is(':checked')){ jQuery(this).parent().siblings().children('input').attr('checked',true);jQuery(this).parent().siblings('input.".esc_js($settings['param_name'])."').val('-1');} else { jQuery(this).parent().siblings().children('input').attr('checked',false);jQuery(this).parent().siblings('input.".esc_js($settings['param_name'])."').val('0');}\" />".esc_html__('All','skudo')."</label>";
			$value = explode(",",$value);
...
if (!function_exists('skudo_partners_categories_settings_field')){
	function skudo_partners_categories_settings_field($settings, $value){
		//$dependency = vc_generate_dependencies_attributes($settings);
...
if (!function_exists('skudo_testimonials_categories_settings_field')){
	function skudo_testimonials_categories_settings_field($settings, $value){
		//$dependency = vc_generate_dependencies_attributes($settings);
...
if (!function_exists('skudo_fa_settings_field')){
	function skudo_fa_settings_field($settings, $value) {
	   //$dependency = vc_generate_dependencies_attributes($settings);
		...
		$output = '<div class="skudo_fa_block">'
		...
		foreach($icons as $i){
			...
			$output .= '" onclick="jQuery(this).closest(\'.edit_form_line\').find(\'input.skudo_fa_field\').val(\''.esc_js($i).'\');jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');"/>';
		}
		$output .= '</div>';
	   return $output;
	}
...
if (!class_exists('VCExtendAddonClass')){
	class VCExtendAddonClass {
	    ...
	    function __construct() {
	        ...
	        add_action( 'vc_before_init', array( $this, 'skudo_integrateWithVC' ) );
 
	        // Use this when creating a shortcode addon
	        if (function_exists('add_shortcode')){
				add_shortcode( 'verticaltabs', array( $this, 'skudo_renderVerticalTabs' ) );
				add_shortcode( 'verticaltab', array( $this, 'skudo_renderVerticalTab' ) );
				add_shortcode( 'testimonials', array( $this, 'skudo_renderTestimonials' ) );
				add_shortcode( 'twitter_scroller', array( $this, 'skudo_renderTwitterScroller' ) );
				add_shortcode( 'partners', array( $this, 'skudo_renderPartners' ) );
				add_shortcode( 'team', array( $this, 'skudo_renderTeam' ) );
				add_shortcode( 'newsletter', array($this, 'skudo_renderNewsletter') );
	        ...
	        //ererere
	        global $skudo_team_index, $skudo_team_index_aux, $skudo_partners_index, $skudo_twitter_index, $skudo_testimonials_index, $skudo_single_testimonial_index, $skudo_vertical_tabs, $skudo_vt_index;
	        	$skudo_team_index = $skudo_team_index_aux = $skudo_partners_index = $skudo_twitter_index = $skudo_testimonials_index = $skudo_single_testimonial_index = $skudo_vertical_tabs = $skudo_vt_index = 1;
	    }
	    ...
	    public function skudo_integrateWithVC() {
	        ...
	        vc_map( array(
	            "name" => esc_html__("[UPPER] Newsletter", 'skudo'),
				"category" => 'UPPER Shortcodes',
	            "description" => esc_html__("Newsletter", 'skudo'),
	            "base" => "newsletter",
	            "icon" => "vc_extend_newsletter_icon", // or css class name which you can reffer in your css file later. Example: "vc_extend_my_class"
	            "category" => esc_html__('UPPER', 'skudo'),
				'show_settings_on_create' => false,
				'category' => array('UPPER Shortcodes',esc_html__('Content','skudo')),
				'params' => array()
		        )
			);
	        ...
	        vc_map( array(
	            "name" => esc_html__("[UPPER] Vertical Tabs", 'skudo'),
	            "description" => esc_html__("Awesome Vertical Tabs", 'skudo'),
	            "base" => "verticaltabs",
	            ...
	            'admin_enqueue_js' => SKUDO_PLG_URL . '/lib/vc_shortcodes/vertical_tabs.js', // This will load js file in the VC backend editor
	            'front_enqueue_js' => SKUDO_PLG_URL . '/lib/vc_shortcodes/vertical_tabs.js', // This will load js file in the VC backend editor
	            'admin_enqueue_css' => SKUDO_PLG_URL . '/lib/vc_shortcodes/vertical_tabs.css', // This will load css file in the VC 
	            'params' => array(
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Widget title', 'skudo' ),
						'param_name' => 'tab_title',
						'description' => esc_html__( 'Enter text which will be used as widget title. Leave blank if no title is needed.', 'skudo' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Style', 'skudo' ),
						'param_name' => 'style_vt',
						'value' => array( 'Icon' => 'icon', 'Text' => 'text', 'Icon + Text' => 'icontext' ),
						'std' => 'icon',
						'description' => esc_html__( 'Choose between just display an icon or icon and text.', 'skudo' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Orientation', 'skudo' ),
						'param_name' => 'orientation',
						'value' => array( 'Vertical' => 'vertical', 'Horizontal' => 'horizontal' ),
						'std' => 'vertical',
						'description' => esc_html__( 'Choose between Vertical and Horizontal orientation (it also affects the effect).', 'skudo' )
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Extra class name', 'skudo' ),
						'param_name' => 'el_class',
						'description' => esc_html__( 'If you wish to style particular content element differently, then use this field to add a class name and then refer to it in your css file.', 'skudo' )
					)
				),
				'custom_markup' => '<div class="wpb_tabs_holder wpb_holder vc_container_for_children"><ul class="tabs_controls"></ul>%content%</div>',
				'default_content' => '[verticaltab title="' . esc_html__( 'Tab 1', 'skudo' ) . '" tab_id="' . $tab_id_1 . '" icon="fa-adjust"][/verticaltab]',
				'js_view' => 'VcVerticalTabsView',
				'category' => array('UPPER Shortcodes',esc_html__('Content','skudo')),
		        )
			);
			
			$upper_icons_set_type = class_exists('Ultimate_VC_Addons') ? "icon_manager" : "skudo_fa";
		
			vc_map( array(
				'name' => esc_html__( '[UPPER] Vertical Tab', 'skudo' ),
				"category" => 'UPPER Shortcodes',
				'base' => 'verticaltab',
				...
				'params' => array(
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Title', 'skudo' ),
						'param_name' => 'title',
						'description' => esc_html__( 'Tab title.', 'skudo' )
					),
					array(
						"type" => $upper_icons_set_type,
						"class" => "",
						"heading" => __("Select Icon ","skudo"),
						"param_name" => "icon",
						"value" => "",
						'description' => esc_html__( 'Choose an Icon', 'skudo' )
					),
/*
					array(
						'type' => 'textarea_html',
...
*/
					array(
						'type' => 'tab_id',
						'heading' => esc_html__( 'Tab ID', 'skudo' ),
						'param_name' => "tab_id"
					)
				),
			...
			vc_map( array(
				'name' => esc_html__( '[UPPER] Testimonials', 'skudo' ),
				"category" => 'UPPER Shortcodes',
				'base' => 'testimonials',
				'is_container' => false,
				...
				'admin_enqueue_js' => SKUDO_PLG_URL . '/lib/vc_shortcodes/testimonials.js',
				'params' => array(
					array(
						'type' => 'dropdown',
						'heading' => esc_html__('Style', 'skudo'),
						'param_name' => 'style_testimonials',
						'description' => esc_html__('3 Different Styles to choose from.','skudo'),
						'value' => array(
							esc_html__( 'Style 1', 'skudo' ) => 'style1',
							esc_html__( 'Style 2 (with scroller)', 'skudo' ) => 'style2',
							esc_html__( 'Style 3 (single wide)', 'skudo' ) => 'style3'
						),
					),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__('Animation Type','skudo'),
						'param_name' => 'des_testimonials_flex_animation',
						'description' => esc_html__('Choose between Slide and Fade effects.','skudo'),
						'value' => array(
							esc_html__( 'Slide', 'skudo' ) => 'slide',
							esc_html__( 'Fade', 'skudo' ) => 'fade'
						),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Slideshow?', 'skudo' ),
						'param_name' => 'des_testimonials_flex_slideshow',
						'description' => esc_html__( 'Animate slider automatically.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Slideshow Speed', 'skudo' ),
						'param_name' => 'des_testimonials_flex_slideshow_speed',
						'description' => esc_html__( 'Set the speed of the slideshow cycling, in milliseconds.', 'skudo' ),
						'value' => '3500'
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Animation Duration', 'skudo' ),
						'param_name' => 'des_testimonials_flex_animation_duration',
						'description' => esc_html__( 'Set the speed of animations, in milliseconds.', 'skudo' ),
						'value' => '1000'
					),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in Desktop', 'skudo' ),
						'param_name' => 'des_testimonials_flex_items_desktop',
						'description' => esc_html__( 'The number of visible items per slide in a desktop.', 'skudo' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in a Small Desktop', 'skudo' ),
						'param_name' => 'des_testimonials_flex_items_small_desktop',
						'description' => esc_html__( 'The number of visible items per slide in a small desktop.', 'skudo' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in a Tablet', 'skudo' ),
						'param_name' => 'des_testimonials_flex_items_tablet',
						'description' => esc_html__( 'The number of visible items per slide in a tablet.', 'skudo' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in Mobile', 'skudo' ),
						'param_name' => 'des_testimonials_flex_items_mobile',
						'description' => esc_html__( 'The number of visible items per slide in a mobile.', 'skudo' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					/* endof new skudo options */
					
					
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Direction Navigation', 'skudo' ),
						'param_name' => 'des_testimonials_flex_direction_nav',
						'description' => esc_html__( 'Create navigation for previous/next navigation.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Navigation Style', 'skudo' ),
						'param_name' => 'des_testimonials_flex_nav_style',
						'description' => esc_html__( 'Choose between Dark and Light style.', 'skudo' ),
						'value' => array( esc_html__( 'Dark', 'skudo' ) => 'dark', esc_html__( 'Light', 'skudo' ) => 'light' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Control Navigation', 'skudo' ),
						'param_name' => 'des_testimonials_flex_control_nav',
						'description' => esc_html__( 'Create navigation for paging control of each slide.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Pause on Hover', 'skudo' ),
						'param_name' => 'des_testimonials_flex_pause_on_hover',
						'description' => esc_html__( 'Pause the slideshow when hovering over slider, then resume when no longer hovering.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Testimonials Scroller Height', 'skudo' ),
						'param_name' => 'des_testimonials_flex_height',
						'description' => esc_html__( 'The height of the testimonials scroller in pixels.', 'skudo' ),
						'std' => '650'
					),
					/*endofflexoptions*/
					...
					array(
						...
						'heading' => esc_html__( 'Categories', 'skudo' ),
						'param_name' => 'testimonials_cats',
						'description' => esc_html__( 'Choose one or more Categories', 'skudo' )
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Number of Testimonials', 'skudo' ),
						'param_name' => 'number',
						'description' => esc_html__( 'The number of testimonials. If set to 0 it will display all.', 'skudo' )
					),
					/* new order options */
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order By', 'skudo' ),
						'param_name' => 'skudo_testimonials_orderby',
						'value' => array( esc_html__( 'Date', 'skudo' ) => 'date', esc_html__( 'Author', 'skudo' ) => 'author', esc_html__( 'Company', 'skudo' ) => 'company' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order', 'skudo' ),
						'param_name' => 'skudo_testimonials_order',
						'value' => array( esc_html__( 'DESC', 'skudo' ) => 'DESC', esc_html__( 'ASC', 'skudo' ) => 'ASC' )
					),
					/* endof new order options */
					array(
						'type' => 'checkbox',
						'heading' => esc_html__( 'Hide author?', 'skudo' ),
						'param_name' => 'hide_author',
						'description' => esc_html__( 'If selected, the author will not be displayed.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes' )
					),
					array(
						'type' => 'checkbox',
						'heading' => esc_html__( 'Hide company?', 'skudo' ),
						'param_name' => 'hide_company',
						'description' => esc_html__( 'If selected, the company will not be displayed.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes' )
					)
				),
				'js_view' => 'VcTestimonialsView',
				'front_enqueue_js' => array(SKUDO_PLG_URL . "/lib/vc_shortcodes/testimonials.js")
			) );
		
			vc_map( array(
				'name' => esc_html__('[UPPER] Twitter Scroller', 'skudo'),
				"category" => 'UPPER Shortcodes',
				'base' => 'twitter_scroller',
				'is_container' => false,
				'icon' => 'vc_extend_twitter_scroller_icon',
				'admin_enqueue_js' => SKUDO_PLG_URL . '/lib/vc_shortcodes/twitter_scroller.js',
				'params' => array(
					/*flexoptions*/
					array(
						'type' => 'dropdown',
				...
				'front_enqueue_js' => array(SKUDO_PLG_URL . "/lib/vc_shortcodes/twitter_scroller.js")
			) );
		
			vc_map( array(
				'name' => esc_html__( '[UPPER] Partners', 'skudo' ),
				'category' => 'UPPER Shortcodes',
				'base' => 'partners',
				'is_container' => false,
				'icon' => 'vc_extend_partners_icon', // or css class name which you can reffer in your css file later. Example: "vc_extend_my_class"
				'admin_enqueue_js' => SKUDO_PLG_URL . '/lib/vc_shortcodes/partners.js',
				'params' => array(
					array(
						'type' => 'partners_cats',
						'heading' => esc_html__( 'Categories', 'skudo' ),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Number of Partners', 'skudo' ),
						'param_name' => 'number',
						'description' => esc_html__( 'The number of Partners. If set to 0 it will display all.', 'skudo' )
					),
					/* new order options */
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order By', 'skudo' ),
						'param_name' => 'skudo_partners_orderby',
						'value' => array( esc_html__( 'Date', 'skudo' ) => 'date', esc_html__( 'Title', 'skudo' ) => 'title' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order', 'skudo' ),
						'param_name' => 'skudo_partners_order',
						'value' => array( esc_html__( 'DESC', 'skudo' ) => 'DESC', esc_html__( 'ASC', 'skudo' ) => 'ASC' )
					),
					/* endof new order options */
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Display tooltip?', 'skudo' ),
						'param_name' => 'tooltip',
						'description' => esc_html__( 'If selected, a tooltip with the Partner\'s name will be displayed on hover.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Display scroller?', 'skudo' ),
						'param_name' => 'scroller',
						'description' => esc_html__( 'If selected, the Partner\'s will be displayed with a scroller.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Auto Play?', 'skudo' ),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Animation Speed', 'skudo' ),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Navigation?', 'skudo' ),
						'param_name' => 'des_partners_owl_navigation',
						'description' => esc_html__( 'Display "next" and "prev" buttons.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Pagination?', 'skudo' ),
						'param_name' => 'des_partners_owl_pagination',
						'description' => esc_html__( 'Show pagination.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Pagination Style', 'skudo' ),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Slider Height', 'skudo' ),
						'param_name' => 'des_partners_owl_height',
						'description' => esc_html__( 'The height of the slider in pixels.', 'skudo' ),
						'value' => '130'
					),
					/*endofowlcarousel*/
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Partners per row', 'skudo' ),
						'param_name' => 'number_per_row',
						'description' => esc_html__( 'The number of Partners per row.', 'skudo' ),
						'value' => array_reverse(array( '6' => '6', '4' => '4', '3' => '3', '2' => '2', '1' => '1' ))
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Row Height', 'skudo' ),
						'param_name' => 'row_height',
						'description' => esc_html__( 'The height of each row of partners in pixels.', 'skudo' ),
						'value' => '130'
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Display Inner Border?', 'skudo' ),
						'param_name' => 'innerborder',
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' ),
						'description' => esc_html__( 'Displays a border between the partners.', 'skudo' ),
					),
					array(
						'type' => 'colorpicker',
						'heading' => esc_html__( 'Inner Border Color', 'skudo' ),
						'param_name' => 'inner_border_color',
						'description' => esc_html__( 'Select a color for the border.', 'skudo' )
					)

				),
				'js_view' => 'VcPartnersView',
				'front_enqueue_js' => array(SKUDO_PLG_URL . "/lib/vc_shortcodes/partners.js")
			) );
		
			vc_map( array(
				'name' => esc_html__( '[UPPER] Team', 'skudo' ),
				"category" => 'UPPER Shortcodes',
				'base' => 'team',
				'is_container' => false,
				"icon" => "vc_extend_team_icon", // or css class name which you can reffer in your css file later. Example: "vc_extend_my_class"
				'admin_enqueue_js' => SKUDO_PLG_URL . '/lib/vc_shortcodes/team.js',
				'params' => array(
					array(
						'type' => 'team_cats',
						'heading' => esc_html__( 'Categories', 'skudo' ),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Number of Team Members', 'skudo' ),
						'param_name' => 'number',
						'description' => esc_html__( 'The number of Team Members. If set to 0 it will display all.', 'skudo' )
					),
					/* new order options */
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order By', 'skudo' ),
						'param_name' => 'skudo_team_orderby',
						'value' => array( esc_html__( 'Date', 'skudo' ) => 'date', esc_html__( 'Title', 'skudo' ) => 'title' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order', 'skudo' ),
						'param_name' => 'skudo_team_order',
						'value' => array( esc_html__( 'DESC', 'skudo' ) => 'DESC', esc_html__( 'ASC', 'skudo' ) => 'ASC' )
					),
					/* endof new order options */
					array(
						...
						'heading' => esc_html__( 'Display tooltip?', 'skudo' ),
						'param_name' => 'tooltip',
						'description' => esc_html__( 'If selected, a tooltip with the Team Member\'s name will be displayed on hover.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Display scroller?', 'skudo' ),
						'param_name' => 'scroller',
						'description' => esc_html__( 'If selected, the Team Member\'s will be displayed with a scroller.', 'skudo' ),
						'value' => array( esc_html__( 'Yes, please', 'skudo' ) => 'yes', esc_html__( 'No, thanks', 'skudo' ) => 'no' )
					),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Team Members per row', 'skudo' ),
						'param_name' => 'number_per_row',
						'description' => esc_html__( 'The number of Partners per row.', 'skudo' ),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Row Height on Desktops', 'skudo' ),
						'param_name' => 'row_height',
						'description' => esc_html__( 'The height of each row of partners in pixels.', 'skudo' ),
						'value' => '350'
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Row Height on Tablets', 'skudo' ),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Row Height on Mobiles', 'skudo' ),
				...
				'js_view' => 'VcTeamView',
				'front_enqueue_js' => array(SKUDO_PLG_URL . "/lib/vc_shortcodes/team.js")
			) );
	    }
	    ...
	    */
	    ...
	    public function skudo_renderNewsletter( $atts, $content = null ){
			$code = str_replace('&', '&amp;', get_option("skudo_mailchimp_code"));
			if (!empty($code)){
			    $output = '<div class="newsletter_shortcode"><div class="mail-box"><div class="mail-news"><div class="news-l"><div class="banner"><h3>'.wp_kses_post(get_option("skudo_newsletter_text")).'</h3><p>'.wp_kses_post(get_option("skudo_newsletter_stext")).'</p></div><div class="form">';
			    if (function_exists('icl_t')){
				    $output = '<div class="newsletter_shortcode"><div class="mail-box"><div class="mail-news"><div class="news-l"><div class="banner"><h3>'.wp_kses_post(icl_t( 'skudo', 'Subscribe our <span class=text_color>Newsletter</span>', get_option('skudo_newsletter_text'))).'</h3><p>'.wp_kses_post(icl_t( 'skudo', 'Subscribe to our newsletter to receive news, cool free stuff updates and new released products (no spam!)', get_option('skudo_newsletter_stext'))).'</p></div><div class="form">';
			    }
				$output .= stripslashes($code);
				$output .= '</div></div></div></div></div>';
			} else {
				$output = '<div class="newsletter_shortcode">'.esc_html__('You need to fill the inputs on the Appearance > Skudo Options > Newsletter panel in order to work.','skudo').'</div>';
			}
		    return $output;
	    }
    
		public function skudo_renderTeam( $atts, $content = null ) {	
  		  extract( shortcode_atts( array(
  			 'team_cats' => -1,
  		  	 'number' => -1,
  			 'tooltip' => 'yes',
  			 'scroller' => 'yes',
  			 'skudo_team_orderby' => 'date',
  			 'skudo_team_order' => 'DESC',
  		  ...
  		  global $skudo_team_index, $skudo_team_index_aux;
			$skudo_team_index = vc_is_inline() ? uniqid() : $skudo_team_index;
			$skudo_team_index_aux = vc_is_inline() ? uniqid() : $skudo_team_index_aux;
  		  ...
  		  $args['orderby'] = $skudo_team_orderby;
  		  $args['order'] = $skudo_team_order;
  		  $team = get_posts($args);
		
  		  if ($scroller == "no"){
  				$output .= '<div id="des-team-'.$skudo_team_index.'" class="des-team-shortcode row team text-center noscroller '; 
  				if ($tooltip == 'yes') $output .= "withtooltip";
  				$output .= '">';
  				foreach ($team as $t){
  					$output .= '<div class="col-xs-'.(12/intval($number_per_row,10)).' col-sm-'.(12/intval($number_per_row,10)).'" style="height: '.intval($row_height,10).'px;"><a data-toggle="modal" href="#member'.$skudo_team_index.'-'.$skudo_team_index_aux.'" class="modal-popup-link team-profile profile-id-'.$t->ID.'"><img src="'; 
					...
					if ( ! empty( $post_custom_css ) ) {
						...
						skudo_set_custom_inline_css($post_custom_css);
					}
  					
  					$skudo_team_index_aux++;
  				}
  				$output .= '</div>';
  				$team_profile_contents = '';
  				$skudo_team_index_aux = 1;
  				foreach ($team as $t){
  					$team_profile_contents .= '<div id="member'.$skudo_team_index.'-'.$skudo_team_index_aux.'" class="modal team_member_profile_content"><div class="container">'.do_shortcode($t->post_content).'</div><a href="#" class="close" data-dismiss="modal">x</a></div>';
  					$skudo_team_index_aux++;
  				...
  				skudo_set_team_profiles_content($team_profile_contents);
  				$output .= '<style>@media only screen and (min-width: 768px) and (max-width: 959px) {#des-team-'.$skudo_team_index.' > div{height:'.intval($row_height_tablets,10).'px !important;}}@media only screen and (max-width: 767px) {#des-team-'.$skudo_team_index.' > div{height:'.intval($row_height_mobiles,10).'px !important;}}</style>';
  		  } else {
  				wp_enqueue_script('ult-slick');
  				wp_enqueue_script('ultimate-appear');
  				wp_enqueue_script('ult-slick-custom');
  				$output .= '<div id="des-team-'.$skudo_team_index.'" class="des-team-shortcode row team text-center withscroller'; 
  				if ($tooltip == 'yes') $output .= " withtooltip";
  				$output .= '" style="max-height:'.intval($des_team_owl_height).'px;">';
  				foreach ($team as $t){
  					$output .= '<div style="display:inline-block;"><a data-toggle="modal" href="#member'.$skudo_team_index.'-'.$skudo_team_index_aux.'" class="modal-popup-link team-profile profile-id-'.esc_attr($t->ID).'"><img src="'; 
  					$image = wp_get_attachment_image_src( get_post_thumbnail_id( $t->ID ), 'single-post-thumbnail' );
  					$output .= esc_url($image[0]);
  				...
  				$skudo_team_index_aux = 1;
  				foreach ($team as $t){
  					$team_profile_contents .= '<div id="member'.$skudo_team_index.'-'.$skudo_team_index_aux.'" class="modal team_member_profile_content"><div class="container">'.wpb_js_remove_wpautop($t->post_content).'</div><a href="#" class="close" data-dismiss="modal">x</a></div>';
  					$skudo_team_index_aux++;
  				}
  				skudo_set_team_profiles_content($team_profile_contents);
  		  }
		...
		public function skudo_renderPartners( $atts, $content = null ) {
		
		  extract( shortcode_atts( array(
			 'partners_cats' => -1,
		  	 'number' => -1,
		  	 'skudo_partners_orderby' => 'date',
		  	 'skudo_partners_order' => 'DESC',
			 'tooltip' => 'yes',
			 'scroller' => 'yes',
		  ...
		  global $skudo_partners_index;
			  $skudo_partners_index = vc_is_inline() ? uniqid() : $skudo_partners_index;
		  ...
		  $args['orderby'] = $skudo_partners_orderby;
  		  $args['order'] = $skudo_partners_order;
		  $partners = get_posts($args);

		  if ($scroller == "no"){
				$output .= '<div id="partners-'.esc_attr($skudo_partners_index).'" class="partners-container noscroller'; 
				if ($tooltip == 'yes') $output .= " withtooltip";
				...
				$output .= '</div>';
				$output .= "<script type=\"text/javascript\">
						jQuery(document).ready(function(){ jQuery('#partners-".esc_js($skudo_partners_index).".withtooltip .partner-item a, #partners-".esc_js($skudo_partners_index).".withtooltip .carousel-item a').tooltip(); });
						</script>";
			} else {
				wp_enqueue_script('ult-slick');
	  			wp_enqueue_script('ultimate-appear');
	  			wp_enqueue_script('ult-slick-custom');
				$output .= '<div id="logos-carousel-'.esc_attr($skudo_partners_index).'" class="partners-container owl-carousel light-text'; 
				...
				<script type=\"text/javascript\">
					jQuery(document).ready(function(){ 
						...
						var who = jQuery('#logos-carousel-".esc_js($skudo_partners_index)."');
		  ...
		  $skudo_partners_index++;
	      return $output;
		}

		public function skudo_renderTwitterScroller ( $atts, $content = null ){
			extract( shortcode_atts( array(
			...
			global $skudo_twitter_index;
				$skudo_twitter_index = vc_is_inline() ? uniqid() : $skudo_twitter_index;
			$output = "";
		
			$errors = false;
			$username = get_option("skudo_twitter_username");
			...
			$tweetcount = get_option('skudo_twitter_number_tweets');
			if (empty($tweetcount)) $tweetcount = 0;
		
			if ($errors){
				$output = '<div class="twitter_warning">'.esc_html__('You need to fill the following Twitter related fields in the Admin Panel > Skudo Options > Twitter and Social Icons > Twitter. ','skudo').'<br/>'.wp_kses_post($errors).'</div>';
			} else {
				wp_enqueue_script('ult-slick');
  				wp_enqueue_script('ultimate-appear');
  				wp_enqueue_script('ult-slick-custom');
				$output = '<div id="des-twitter-'.esc_attr($skudo_twitter_index).'" class="twitter-container style-'.esc_attr($des_twitter_direction_nav_style).'" rel="'.esc_attr($username.'|'.$tweetcount).'"><div class="icon-author animated flipInX"><div class="bird"><i class="fa fa-twitter"></i></div><p class="twitter-author"><a href="http://twitter.com/'.esc_attr($username).'" target="_blank"><b>'.sprintf(esc_html__("%s",'skudo'), get_option("skudo_twitter_follow_us")).'</b></a></p></div><div class="twitter-slider"><div id="twitter-feed"></div></div></div>';
				$upper_twitter_pre_tweet = sprintf(esc_html__("%s", "skudo"), get_option("skudo_twitter_pre_tweet"));
				if (function_exists('icl_t')){
					$output = '<div id="des-twitter-'.esc_attr($skudo_twitter_index).'" class="twitter-container style-'.esc_attr($des_twitter_direction_nav_style).'" rel="'.esc_attr($username.'|'.$tweetcount).'"><div class="icon-author animated flipInX"><div class="bird"><i class="fa fa-twitter"></i></div><p class="twitter-author"><a href="http://twitter.com/'.esc_attr($username).'" target="_blank"><b>'.sprintf(esc_html__("%s",'skudo'), icl_t( 'skudo', 'Follow us on Twitter', get_option('skudo_twitter_follow_us'))).'</b></a></p></div><div class="twitter-slider"><div id="twitter-feed"></div></div></div>';
					$upper_twitter_pre_tweet = sprintf(esc_html__("%s", "skudo"), icl_t( 'skudo', 'I was looking at ', get_option('skudo_twitter_pre_tweet')));
				}
				...
				$output .= "<script type=\"text/javascript\">
					jQuery(document).ready(function(){
						...
						var who = jQuery('#des-twitter-".esc_js($skudo_twitter_index)."');
						if (typeof vc !== 'undefined') who = jQuery(vc.".'$frame_body'.").find('#des-twitter-".esc_js($skudo_twitter_index)."');
	    ...
	    public function skudo_renderTestimonials( $atts, $content = null ) {	
		  ...
		  global $skudo_testimonials_index, $skudo_single_testimonial_index;
			  $skudo_testimonials_index = vc_is_inline() ? uniqid() : $skudo_testimonials_index;
			  $skudo_single_testimonial_index = vc_is_inline() ? uniqid() : $skudo_single_testimonial_index;
		  $output = "";
		  ...
		  if ($style_testimonials === "style1"){
			  $output .= '<div id="testimonials-container-'.esc_attr($skudo_testimonials_index).'" class="container testimonials '.esc_attr($style_testimonials).'"><div class="testimonial-box">'; 
			  
			  $first = true;
			  $skudo_single_testimonial_index = 1;

			  $output .= '<div class="testimonials-content">';
			  foreach ($testimonials as $t){
				  	$active = ($skudo_single_testimonial_index == 1) ? " active" : "";
					$output .= '<div class="testimonial'.esc_attr($active).'" id="testimonial-'.esc_attr($skudo_single_testimonial_index).'">'.wp_kses_post($t->post_content);
					...
					if ($hide_author === "no" && $hide_company === "no" || get_post_meta($t->ID,'author_value', true) != '' && get_post_meta($t->ID,'company_value', true) != '') { $output .= '</span>'; }
					$output .= '</div>';
				    $skudo_single_testimonial_index++;
			  }
			  ...
			  foreach ($testimonials as $t){
				$output .= '<li><a href="#testimonial-'.esc_attr($skudo_single_testimonial_index).'" ';
				if ($first){
					$first = false;
					$output .= ' class="active" ';
				}
			  ...
			  <script type=\"text/javascript\">
			  jQuery(document).ready(function(){
					var who = jQuery('#testimonials-container-".esc_js($skudo_testimonials_index)."');
			...
			if ($style_testimonials == "style2"){
				$output .= '<div id="testimonials-slider-'.esc_attr($skudo_testimonials_index).'" class="testimonials-style2 style-'.esc_attr($des_testimonials_flex_nav_style).'"><ul class="slides styled-list"'; 
			} else {
				$output .= '<div id="testimonials-slider-'.esc_attr($skudo_testimonials_index).'" class="single-wide-testimonials testimonials-style2 style-'.esc_attr($des_testimonials_flex_nav_style).'"><ul class="slides styled-list"'; 
			}
			...
			<script type=\"text/javascript\">
				jQuery(document).ready(function(){
					...
					var who = jQuery('#testimonials-slider-".esc_js($skudo_testimonials_index)." ul.slides');
		  ...
		  $skudo_testimonials_index++;
	      return $output;
		}

	    public function skudo_renderVerticalTabs( $atts, $content = null ) {
	      extract( shortcode_atts( array(
	        'title' => '',
	        'style_vt' => 'icon',
		  ...
		  global $skudo_vertical_tabs;
			  $skudo_vertical_tabs = vc_is_inline() ? uniqid() : $skudo_vertical_tabs;
		...
		public function skudo_renderVerticalTab( $atts, $content = '[vc_row_inner][vc_column_inner][/vc_column_inner][/vc_row_inner]' ) {
	      extract( shortcode_atts( array(
	        'title' => 'Tab',
			'icon' => 'fa-adjust',
	        'tab_id' => ''
		  ...
		  global $skudo_vt_index;
			  $skudo_vt_index = vc_is_inline() ? uniqid() : $skudo_vt_index;
	        
		  $output = "<div class='label {$skudo_vt_index}'><div class='skudo_icon_special_tabs'><i class='fa {$icon}'></i></div><div class='title'><a>{$title}</a></div><div class='divider-vertical-tabs'></div></div><div class='content {$skudo_vt_index}'>{$content}</div>";
...
if (!function_exists('skudo_vc_before_init')){
	function skudo_vc_before_init(){

		if (function_exists('vc_add_shortcode_param')){
			vc_add_shortcode_param('team_cats', 'skudo_team_categories_settings_field');
			vc_add_shortcode_param('partners_cats', 'skudo_partners_categories_settings_field');
			vc_add_shortcode_param('testimonials_cats', 'skudo_testimonials_categories_settings_field');
			vc_add_shortcode_param('skudo_fa', 'skudo_fa_settings_field');	
		}
					...
					foreach ( $this->settings['params'] as $param ) {
						if ( $param['param_name'] != 'content' ) {
							if ( isset( $param['value'] ) && is_string( $param['value'] ) ) {
								$shortcode_attributes[$param['param_name']] = esc_html__( $param['value'], "skudo" );
							} elseif ( isset( $param['value'] ) ) {
								$shortcode_attributes[$param['param_name']] = $param['value'];
							}
						} else if ( $param['param_name'] == 'content' && $content == NULL ) {
							$content = esc_html__( $param['value'], "skudo" );
						}
					}
		...
		if (class_exists('WPBakeryVisualComposerAbstract')){
			if (!function_exists('upper_vc_library_cat_list')){
				function upper_vc_library_cat_list() {
					return array( __('All','skudo') => 'all', 
						__('About','skudo') => 'about', 
						__('FAQ','skudo') => 'faq',
						__('Counters','skudo') => 'counters',  
						__('Contact & Maps','skudo') => 'contactforms', 						
						__('General','skudo') => 'general',  
						__('Portfolio','skudo') => 'portfolio',
						__('Pricing','skudo') => 'pricing',
						__('Services','skudo') => 'services',
						__('Stylish Tabs','skudo') => 'verticalstabsicon', 
						__('Testimonials','skudo') => 'testimonials',
						__('Team','skudo') => 'team');
				}
			}
		
			if(!function_exists('add_upper_studio_to_vc')) {
				function add_upper_studio_to_vc() {
					if (is_admin()) { 
						require_once ('lib/upper-studio/upper-studio-templates.php');
						wp_enqueue_style( 'upper-studio', plugins_url('lib/upper-studio/upper-studio.css', __FILE__), 'all' );
					}
				}
			}
		
			add_upper_studio_to_vc();
		}
...
add_action('vc_before_init', 'skudo_vc_before_init');

if (!function_exists('skudo_add_increase_time')){
	function skudo_add_increase_time(){
...
if (!function_exists('upper_scrape_instagram')){
	function upper_scrape_instagram() {
		...
		$title = get_option('skudo_instagram_title', '');
		$limit = get_option('skudo_instagram_limit', 9);
		$uid = get_option('skudo_insta_uid', false);
		$target = get_option('skudo_instagram_target', '_self');
		$access_token = get_option('skudo_insta_token', false);
		
		if (!$access_token || !$uid){
			echo 'Please authorize Instagram on Appearance > Skudo Options > Social Networks > Instagram and clicking on Authorize Instagram.';
			return;
		}
		...
		$response = skudo_remote_get( $url, $args, "_scrapper", $limit );
		...
		if (isset($response['data'])){
			...
			$username = '';
			if (get_option('skudo_enable_instagram_grayscale') == "on") $imgclass .= ' skudo_grayscale ';
			...
			if ( get_option('skudo_instagram_link') != '' ) {
				?><p class="<?php echo esc_attr( $linkclass ); ?>"><a href="<?php echo trailingslashit( '//ins'.'tag'.'ram.com/' . $username ); ?>" rel="me" target="<?php echo esc_attr( get_option('skudo_instagram_target') ); ?>"><?php echo esc_html( get_option('skudo_instagram_link') ); ?></a></p><?php
			}
			
		} else {
			echo 'Something went wrong. Please re-authorize Instagram on Appearance > Skudo Options > Social Networks > Instagram and clicking on Authorize Instagram.';
			return;
		}
	}
}

if (!function_exists('skudo_remote_get')){
	function skudo_remote_get($url = null, $args = array(), $culprit = "", $limit = 12) {
		if (!get_transient( 'skudo_insta_transient'.$culprit )){
			$url = add_query_arg($args, trailingslashit($url));
			$response = skudo_validate_response(wp_remote_get($url, array('timeout' => 29)));
			if (!isset($reponse['error'])) set_transient( 'skudo_insta_transient'.$culprit, $response, 3600 );
		} else {
			$response = get_transient( 'skudo_insta_transient'.$culprit );
...
if (!function_exists('skudo_validate_response')){
	function skudo_validate_response($json = null) {
		if (!($response = json_decode(wp_remote_retrieve_body($json), true)) || 200 !== wp_remote_retrieve_response_code($json)) {
			...
			if (is_wp_error($json)) {
				...
				$response = array(
					'error' => 1,
					'message' => esc_html__('Unknow error occurred, please try again', 'skudo')
				);
			}
		}
		return $response;
...
if (!function_exists('upper_insta_cron_exec')){
	function upper_insta_cron_exec(){
		$at = get_option( 'skudo_insta_token', false );
		if ($at != false){
			$args = array(
			...
			$response = skudo_validate_response( wp_remote_get( $url_with_args, array('timeout' => 29) ) );
			if (isset($response['access_token'])) update_option('skudo_insta_token', $response['access_token']);
		}
	}
}
